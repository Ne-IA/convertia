...
## P11.1 — RC assembly & freeze (the artifact under test)
...
- [ ] **P11.1.1** [RELEASE] Cut the RC tag and trigger the Lane-B release pipeline against it · §6.7.2 · G56b
...
- [ ] **P11.1.2** [RELEASE] Collect the per-platform RC artifacts + `reliability-report.json` as the verification inputs · §6.1.2 §6.5.2 · G30 G31
  needs: P11.1.1
...
- [ ] **P11.1.3** [DOC] Record the RC acceptance manifest (tag, SHAs, evidence index) as the sign-off anchor · §6.10
  needs: P11.1.2
  > stand up the tracked acceptance manifest (the RC tag, the per-platform artifact SHAs from P11.1.2, and an index pointing at each evidence artifact — the ledger, the egress-run log, the size-asset line, `docs/usability-floor.md`) so the final RC sign-off (P11.7) checks off against one auditable record rather than scattered run logs. The §6.10 ref homes the DoD-traceability the manifest mirrors.

---

## P11.2 — Cross-platform E2E test matrix (the automated flow gate)
...
- [ ] **P11.2.1** [TEST] Verify the Windows headed-E2E flow passes on the RC (tauri-driver + WebdriverIO) · §6.4.6 · G33a
  needs: P11.1.2
...
- [ ] **P11.2.2** [TEST] Verify the Linux headed-E2E flow passes on the RC under Xvfb (extracted AppImage ELF) · §6.4.6 · G33a
  needs: P11.1.2
...
- [ ] **P11.2.3** [TEST] Verify the macOS degraded-smoke leg passes on the RC (launch + synthetic-argv conversion + exit 0) · §6.4.6 · G30
  needs: P11.1.2
...
- [ ] **P11.2.4** [TEST] Assert the E2E matrix is COMPLETE — every required platform leg ran and reported · §6.4.6 §6.4.4 · G30
  needs: P11.2.1, P11.2.2, P11.2.3
...
## P11.3 — §6.6 usability-floor walkthrough (six separately-faileable sub-gates)
...
> `docs/usability-floor.md` (the required v1 artifact, P11.3.5); a failure on any one
> fails the floor for its platform/aspect only.

- [ ] **P11.3.1** [TEST] Run the non-developer conversion walkthrough — named conversions, two-click common path · §6.6
  needs: P11.1.2
...
- [ ] **P11.3.2** [TEST] Run the keyboard-only pass over the core path · §6.6 §5.10
  needs: P11.1.2
  > sub-gate (b): at least one walkthrough completes the core path **keyboard-only** per the §5.10 shortcut map (drag/drop+picker+keyboard reach the same result), and verifies readable contrast/text-size by eye — the human half of the DoD basic-a11y gate, complementing the automated axe-core assertions (§6.4.6a / P11.4.6). Recorded in `docs/usability-floor.md`.
- [ ] **P11.3.3** [TEST] Run the screen-reader smoke pass over Idle→Collecting→Confirm→Converting→Summary · §6.6 §5.6.1
  needs: P11.1.2
  > sub-gate (c): one screen-reader walkthrough on ≥1 platform with the native SR (VoiceOver/NVDA/Orca) steps the §5.6.1(3) per-state traversal table and confirms, against §5.6.1(1)/(2): every state has a reachable non-orphaned landing element; the collected summary + confirm-gate string announce assertively; progress milestones announce (not every tick); decision dialogs announce as `alertdialog` with their accessible name; lossy/divert notes announce politely. This is the verification gate for the §5.6.1 implementable SR contract — axe-core (P11.4.6) proves ARIA validity, not usable announcement. Records which SR + which platform in `docs/usability-floor.md`; feeds §6.10 DoD row 6.
- [ ] **P11.3.4** [TEST] Run the mandatory macOS Sequoia first-launch + per-sidecar quarantine recovery sub-test · §6.6 §7.2.4 · G46
  needs: P11.1.2
  > sub-gate (d): the macOS walkthrough runs on **Sequoia (15.x)** from a freshly **downloaded (quarantined)** RC artifact and the tester succeeds at **both** (1) blocked-app first-launch recovery (Privacy & Security → "Open Anyway", no Control-click bypass on Sequoia) **and** (2) the first-conversion step where an independently-quarantined sidecar may be blocked — confirming the in-app `QuarantinedByOs` guidance (§2.8/§7.2.4, the message naming the specific blocked sidecar) and the §6.2.4 download-page steps actually get a non-technical user through. A stuck-at-Gatekeeper / silent first-conversion failure fails the macOS floor → revisit unsigned posture / guidance copy → re-walk. Preferentially staffed by a non-dev tester (highest non-technical-user blocker). G46's `QuarantinedByOs`-distinct-kind acceptance is the automated complement (P11.4.3).
- [ ] **P11.3.5** [DOC] Author the `docs/usability-floor.md` artifact + assert its machine-checkable staleness criterion against the RC tag · §6.6 · G44
  needs: P11.3.1, P11.3.2, P11.3.3, P11.3.4, P11.3.6
...
- [ ] **P11.3.6** [TEST] Run the single-instance double-extract macOS sub-test · §6.6 §7.1.1
  needs: P11.1.2
...
- [ ] **P11.3.7** [TEST] Assert the §6.6 floor is COMPLETE — all named conversions passed + all platform walkthroughs recorded · §6.6 · G44
  needs: P11.3.5
...
## P11.4 — Definition-of-Done verification against the SSOT
...
- [ ] **P11.4.1** [TEST] Verify the §6.5 reliability gate is GREEN on the RC — every v1 pair `reliable` on all available platforms · §6.5.1 §6.5.2 · G31
  needs: P11.1.2
...
- [ ] **P11.4.2** [TEST] Verify exceptions 1/2 are recorded — `unavailable-per-§3.4`/`demoted` cells ⇔ `docs/demoted-pairs.md` rows · §6.5.3 · G44
  needs: P11.4.1
...
- [ ] **P11.4.3** [TEST] Verify §7.2.3 startup integrity & engine-presence — missing/corrupt engine yields an app-fault, not a crash (DoD gate 19) · §7.2.3 §2.13 · G46
  needs: P11.1.2
...
- [ ] **P11.4.4** [TEST] Verify the offline-egress observability gate is GREEN on the RC — zero outbound + no out-of-input read · §6.7.3 §2.11.4 · G42 G42b
  needs: P11.1.2
...
- [ ] **P11.4.5** [TEST] Verify the §7.2.2 zero-startup-network assertion holds on the RC · §7.2.2 §2.11 · G42
  needs: P11.4.4
  > assert the RC adds **zero** network activity at startup (no update check, no license/telemetry beacon, no font/asset fetch — all bundled, CSP forbids remote origins) inside the egress-deny window — the startup-specific leg of the offline invariant, distinct from the conversion-path egress proof in P11.4.4. The packet-monitor harness is built in P9; this box confirms the RC's startup is silent.
- [ ] **P11.4.6** [TEST] Verify the automated a11y assertions are GREEN on the RC — ARIA/focus (jsdom) + WCAG-AA contrast (live WebView) · §6.4.6a · G33a G33b
  needs: P11.2.1, P11.2.2
  > assert the §6.4.6a automated a11y legs pass on the RC: the Lane-A jsdom `vitest-axe` leg (ARIA-role/state validity + focus-order/roving-tabindex sanity) **and** the Lane-B `@axe-core/webdriverio` WCAG-2.1-AA `color-contrast` session (≥4.5:1 text, ≥3:1 large/UI, **both** themes) on the Linux + Windows legs. The macOS contrast gap is human-covered by the P11.3.2 readable-contrast check (recorded in `docs/usability-floor.md`), not silently skipped. → the automated half of §6.10 DoD row 6; the human halves are P11.3.2 (keyboard/contrast/text-size) and P11.3.3 (SR).
- [ ] **P11.4.7** [TEST] Verify the ≤400 MB compressed artifact-size gate is GREEN on the RC (row 22) · §6.7.2 §3.9.2 · G41
  needs: P11.1.2
...
- [ ] **P11.4.8** [TEST] Verify the §6.10 row 21 no-system-pollution gate is GREEN on the RC · §6.10 · G43
  needs: P11.1.2
...
- [ ] **P11.4.9** [TEST] Verify DoD gate 20 — OS-intake routes through the freeze funnel + no file-association pollution · §7.8.2 §6.10 · G43
  needs: P11.1.2
  > assert the §6.10-row-20 launch-with-files E2E: an Open-with / launch-args invocation routes through the single §7.8 intake funnel (the UI enters Collecting at startup) **and** the §7.8.2 explicit negatives hold (no file-association registered, no custom URL scheme) — the intake-side complement to P11.4.8's no-pollution audit. The intake funnel is built in P2 and the negatives are asserted by the P10 pollution gate; this box proves both on the RC.
- [ ] **P11.4.10** [TEST] Verify DoD gate 18 — single-instance + run-identity (no cross-instance temp clobber) on the RC · §7.1.1 §6.10 · G31
  needs: P11.1.2
  > assert the §6.10-row-18 single-instance + run-identity behaviour on the RC: a second launch hands off to the running instance (§7.1.1) and per-run/per-instance temp ownership + advisory-lock liveness hold so a second launch cannot clobber the first's scratch or interrupt its freeze. Complements the macOS double-extract human sub-test (P11.3.6) with the automated property/behaviour assertion built in P2/P3; this box proves it on the RC.
- [ ] **P11.4.11** [TEST] Verify DoD gate 8 — unwritable/ephemeral-location divert fallback on the RC · §6.10 · G31
  needs: P11.1.2
...
- [ ] **P11.4.12** [TEST] Verify the remaining release-blocking gates are GREEN — governance-completeness, name-clearance, release-artifact completeness · §6.8 §6.9 · G44 G45 G58
  needs: P11.4.2, P11.4.7
  > assert the P10-built release-blocking meta-gates passed for the RC: **G44** governance-completeness (every required governance doc present + non-stub, incl. the literal-form minisign verify-recipe + the libfuse2/WebView2/Sequoia prerequisite notes + the §6.5.3 demoted-pairs bijection from P11.4.2); **G45** name/trademark clearance-record (`docs/name-clearance.md` present, dated for the RC line, verdict = clear; old-name grep clean); **G58** release-artifact completeness meta-gate (every required asset present — per-OS bundle, `SHA256SUMS`, `.minisig`, SBOM, dated open-CVE report, NOTICE/THIRD-PARTY-LICENSES, copyleft corresponding-source bundle, measured-sizes asset, `usability-floor.md`, `name-clearance.md`, CHANGELOG/release-notes, the Sigstore bundle — and every enumerated asset has a `SHA256SUMS` line). → §6.10 DoD rows 9/10/12/13.

---

## P11.5 — DoD-traceability completeness (every §6.10 row demonstrated)
...
- [ ] **P11.5.1** [TEST] Assert every §6.10 in-scope-gate row maps to a green RC evidence artifact (no unproven DoD row) · §6.10
  needs: P11.4.12, P11.3.7, P11.2.4
  > a completeness check over the §6.10 DoD-traceability table: for each **in-scope-gate** row (1–23), assert a corresponding P11 verification box passed and its evidence is indexed in the P11.1.3 acceptance manifest — so a DoD row cannot be silently unverified at RC. Confirms the §6.10 "every clause has a §6 mechanism" invariant holds *and was actually run* for this RC (not merely that the table is internally consistent — that is plan-lint/spec-lint's job).
- [ ] **P11.5.2** [DOC] Assert the explicit non-gates are correctly excluded — visual polish + engine-currency are not RC blockers · §6.10
  needs: P11.5.1
...
## P11.6 — Engine-bump re-validation readiness (the SSOT continuity gate)
...
- [ ] **P11.7.1** [RELEASE] Sign off the RC — every DoD gate green, every §6.6 sub-gate passed, evidence indexed · §6.10 §6.5 · G58
  needs: P11.5.2, P11.6.1
  > the RC sign-off: assert that the P11.1.3 acceptance manifest shows **all** of — the cross-platform E2E matrix green (P11.2), all six §6.6 usability-floor sub-gates passed (P11.3), every §6.10 in-scope DoD gate verified green on the RC (P11.4–P11.5: reliability §6.5, startup-integrity §7.2.3, offline-egress, ≤400 MB size, no-system-pollution, governance/clearance, artifact-completeness), and the engine-set/continuity check (P11.6) — then record the dated sign-off line against the RC tag. This is the SSOT *v1 DoD* satisfied end-to-end; the release may publish. → the final acceptance box of P1–P11.
