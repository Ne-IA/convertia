...
- [ ] **P10.16** [RELEASE] Derive + assert the static-link SBOM closure for the imgworker stack · §6.3.1 §3.6 · G35a
  needs: P10.14
  > the `convertia-imgworker` STATICALLY links libvips+libheif+libde265+librsvg+cgif+libimagequant+ImageMagick and the C build closure the link absorbs (none in `Cargo.lock`; the P4.89 `sub-component` rows name them — librsvg included: the SVG path calls `rsvg::Loader` directly rather than libvips' `svgload`, §3.5.5 control 1, but the librsvg library is still statically absorbed with its own `engines.lock` row, §3.5.5 control 2 / §3.7.2 item 4) — x265 is NOT statically linked: it is the dynamically-loaded libheif encoder plugin (§3.6.1 `[DECIDED]`, `linkage = "plugin-loaded"`, its own P5.67 row) [corrected 2026-09-09 — Co-Pilot: the earlier "+x265" contradicted spec §3.6.1/§3.1 row 1a; spec > plan; build-gates.md's G35 and G35a rows carry the same "+x265" enumeration and are the owed owner-acked L(-1) mirror of this correction], so the G35 static-link-blind-spot is closed by deriving the static-link closure (G35a) and asserting every statically-linked component appears as an SBOM component — the release-tier execution of the P0.7.9-adjacent derived-closure policy.
