...
- [ ] **P10.10** [BUILD] Execute the macOS per-sidecar `lipo -create` universal-fat-binary assembly + the per-sidecar `lipo -info` assertion · §6.1.3 §6.1.4 · G30
  needs: P10.9
  > on the macOS leg, `stage-engines` restores **both** the `aarch64-apple-darwin` AND `x86_64-apple-darwin` slices per sidecar/lib (dual-arch cache keys) and `lipo -create`s each into one `<name>-universal-apple-darwin` fat Mach-O **before** `tauri build` (Tauri does NOT lipo sidecars). The G30 per-sidecar `lipo -info` fat-Mach-O assertion (P0.4.10) fails the leg if a slice is missing rather than shipping a single-arch sidecar that crashes on the other arch.
...
- [ ] **P10.35** [RELEASE] Finalize `docs/reproduce.md` + the build-environment lock (the human rebuild recipe) · §6.2.5 §3.8 · G60
  needs: P0.7.10, P10.8
  > fill the P0.7.10 `docs/reproduce.md` rebuild recipe + build-environment lock (pinned base-image digest, `rust-toolchain.toml`, Tauri CLI/bundler digest, exact build command, expected per-file SHA-256 of the Rust core) an independent party can follow; apply the cheap determinism measures (`SOURCE_DATE_EPOCH` where honoured, recorded toolchain/engine versions, §6.2.5).
...
- [ ] **P10.40** [DOC] Author the five SSOT-mandated governance docs (owner-authored content) · §6.8 · G44
  > the blocking Phase-3 authoring of the five governance docs the §6.8 gate checks: `CONTRIBUTING.md` (inbound=outbound MIT, no CLA, optional DCO, inbound-warranty clause, the quality-bar list stated directly — not by reference to the private `CLAUDE.md`), `CODE_OF_CONDUCT.md` (Contributor-Covenant-class + enforcement contact), `SECURITY.md` (private-advisory channel + the §0.11 untrusted-decoder scope + §2.12 isolation + no-SLA + §7.5-redacted repro), `PRIVACY.md` (restates §2.11 offline / no-telemetry / the cloud-sync caveat), `TRADEMARK.md` (MIT covers code not the name/logo, fork-rename rule, nominative use). (`LICENSE`/`NOTICE`/`THIRD-PARTY-LICENSES` are the P1/P10.18 set.)
