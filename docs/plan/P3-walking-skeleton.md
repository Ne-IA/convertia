...
- [ ] **P3.5** [RUST] Define the `Engine` trait `plan()` for the native CSV/TSV engine (Pure, no I/O) · §3.2 §3.5.6
  ...
  > **[Reconcile: P3.5 — the minimal trait home; P4.1 EXPANDS it]** this box authors the `Engine` TRAIT itself in its minimal walking-skeleton form (the §3.2.2 `plan()` signature returning `Result<Invocation, PlanError>`) together with the native impl; P4.1 (`needs: P3.75`) EXPANDS the same trait to the full §3.2.2 surface (descriptor/capabilities/plan_encode/classify_failure) — never a second trait. `Invocation`/`PlanError`/`ProgressModel::InProcessFraction` come from the P3.4 hull.
