...
- [ ] **P3.45** [RUST] Build the §1.7 wall-clock timeout + wedged-uninterruptible-read bound · §1.7 §0.9 §2.12.4
  needs: P3.44
  > a §0.9-owned wall-clock timeout (tight for this light engine) wraps the sync call; on expiry the loop is cancelled cooperatively → `Failed(EngineHang)`, run **CONTINUES**. The wedged-uninterruptible-read caveat: the abandoned thread MUST NOT exhaust the `spawn_blocking` pool — the pool is **bounded with headroom** above the global degree, AND/OR reads go through a **bounded chunked reader with a short per-read deadline**, so a handful of wedged reads degrade gracefully (those items fail, the batch finishes). In the P3 slice, the **§1.7 wall-clock timeout + the bounded-chunked-read per-read deadline** bound runaway CSV-expansion (in-core untrusted-byte but pure bounded Rust, §2.12.4); the §1.10 input-size *budget* proper (the est-output/scratch-byte ceilings) arrives with the §1.10 engine **P4.72** (not built in P3 — see the P3.49 §1.10-seam slice-stub note), which is then validated/calibrated in P9.41.
...
- [ ] **P3.49** [RUST] Implement C1 `drain_intake` / C3 `get_targets` / C4 `plan_output` for the slice · §0.4.1 §1.3 §1.4 §1.5
  needs: P3.32, P3.29, P3.40, P2.64, P3.78
...
- [ ] **P3.51** [RUST] Wire C9 `open_path` live — `AppHandle` + `RunResult` from `State<RunResultStore>` + the P2.101–103 membership gate + `OpenerExt` invoke + refusal log + `Ok(())` + E2E · §0.4.1 §7.7.1 §7.7.3 §7.5 §2.7.4
...
- [ ] **P3.61** [TEST] Author the CSV→TSV / TSV→CSV corpus fixtures + the bound-firing CSV-expansion fixture · §6.4.5 §6.4.2 · G24a G16
  needs: P3.42, P0.5.11
  > committed corpus fixtures for both pairs (incl. a Windows-1252-encoded CSV, a quoted-field/embedded-delimiter CSV, a CSV-injection `=…`/`@…` leading-token file, a CJK/RTL-content file) under the §6.4.5 conventions; a deterministic **bound-firing** fixture exercising the P3-slice CSV-expansion bound — the **§1.7 wall-clock timeout + the bounded-chunked-read per-read deadline** (P3.45), NOT the §1.10 input-size budget (which arrives with the §1.10 engine P4.72 and is corpus-validated in P9.41 — the P3.49 §1.10-seam slice-stub note). Each fixture's SHA-256 joins the §6.4.2 corpus manifest **via the `stage-corpus` generator (P0.5.11) in the same commit** (G24a integrity, P0.5.4 — never a hand-computed hash); single-source helper, no inline duplication (§6.4.2). (`needs: P0.5.11` for the manifest generator the corpus landing invokes.)
...
> **The owner ruling (2026-07-06): no FS path ever crosses the IPC wire or an `app://` event, in either direction** — the wire carries **IDs + lossy display strings** only; the real `PathBuf`s live in core-side registries (`FrozenCollectedSet`, `RunResultStore`, the `DestinationId` session registry). This cluster owns the revision: P3.76 re-types the wire, P3.77 moves the native drop core-side behind the payload-less `app://intake` nudge, P3.78 renames C1 to the universal `drain_intake` completion door, P3.79/P3.80 re-key C9 and the destination legs to ID-resolution, and P3.81 re-wires the frontend. Placed after P3.74 per the file's established end-of-phase convention so the ids stay gap-free (`.76`–`.81`) rather than re-numbering P3 + every cross-phase reference; the P3.49/P3.51/P3.53/P3.54 consumers carry the forward `needs:` edges onto this cluster, resolved in place by DECISION C (the P3.75 sweep box stays the physically-last section — its "every other P3 box" mandate auto-covers these boxes).

- [ ] **P3.76** [RUST] Build the core-owned-path wire layer — the display-string projections + `DestinationId` session registry + retire `IntakePayload` · §0.6 §0.4.3 §2.10.1
  needs: P2.4, P2.7, P2.12
  > re-type every outbound wire field to *Display strings (`DroppedItem { item, displayName, relPathDisplay? }`, `SkippedItem`, the roots, `OutputPlanPreview`, `DestinationResolved`, `RunResult`, `ItemResult`, `CleanupResidue`, `ItemOutcome::Succeeded`, `ItemStarted`, `IpcError.pathDisplay`/`residueDisplay`); the real `PathBuf`s stay core-side in `FrozenCollectedSet`/`RunResultStore`; stand up the minimal `DestinationId` session registry for core-picked roots; retire `IntakePayload` (no inbound path DTO survives); regenerate `bindings.ts` (G19). The §2.10.1 byte-for-byte filename guarantees hold on the core-side `PathBuf`s — a *Display string is the lossy presentation projection only, never re-submitted as input.
- [ ] **P3.77** [RUST] Move the native drop core-side + the payload-less `app://intake` nudge — `WindowEvent::DragDrop` → the §7.8.1 funnel; `PendingIntake` = the single hand-off buffer · §7.8.1 §5.4 §7.1.1
  needs: P3.76, P2.55, P2.58
  > the native drop is handled in Rust (`WindowEvent::DragDrop`) and funnels into the §7.8.1 intake funnel exactly like launch-arg/second-instance paths — no drop path ever reaches the WebView; `app://intake` becomes a payload-less nudge and `PendingIntake` (P2.58) becomes the single hand-off buffer for ALL intake origins (drop, picker, launch-arg, second-instance), the P2.55 refuse-busy gate unchanged upstream of it.
- [ ] **P3.78** [RUST] Rename C1 → `drain_intake { collectingId, onScan }` — the universal intake-completion door (walk+freeze inside; golden + bindings + contract tests in the same commit) · §0.4.1 §1.1
  needs: P3.77, P2.60
  > C1's `paths`/`origin`/`drainPending` args are retired entirely (the P2.60 drain flag is subsumed — draining `PendingIntake` IS the command): the WebView calls `drain_intake { collectingId, onScan }` on the nudge, the core runs the §1.1 walk + freeze over the buffered core-side paths and returns `CollectedSet`; C2a `pick_for_intake` sheds its `collectingId`/`onScan` args in the same move (per the revised §0.4.1 row — the picker only fills the §7.8.1 funnel and returns `()`; C1 is the sole `onScan` carrier); the C1/C2a golden + `bindings.ts` + contract tests are updated in the same commit.
- [ ] **P3.79** [RUST] Re-key C9 to `OpenTarget` ID-resolution against `State<RunResultStore>` · §0.4.1 §7.7.2 §7.7.3
...
- [ ] **P3.80** [RUST] Re-key the destination legs — C2b returns `{ destination: DestinationId, display }`; `DestinationChoice::ChosenRoot(DestinationId)`; core-resolved persisted last-destination · §0.4.1 §5.8 §7.4
  needs: P3.76, P2.23
...
- [ ] **P3.81** [UI] Re-wire the frontend to the ID+display wire — drop wiring removed, nudge→drain consumption, *Display rendering, C9-by-target · §5.3 §5.4 §5.8
  needs: P3.76, P3.77, P3.78, P3.79, P3.80, P2.120
  > remove the WebView drop wiring (the DropZone keeps only the drag-over affordance rendering — the drop itself is core-side, P3.77); consume the payload-less `app://intake` nudge by calling `drain_intake` (P3.78); render the *Display strings everywhere a path was shown (P3.76); invoke C9 by `OpenTarget` (P3.79) and the destination legs by `DestinationId` + display (P3.80); the regenerated `bindings.ts` stays the only IPC door (§0.4.5).
