...
- [ ] **P7.4** [RUST] Wire the LibreOffice invocation through the §2.12 isolation boundary with the minimal-env + loader-strip + cwd=scratch contract · §3.5.2 §2.12 §2.14 · G29
  needs: P7.1, P4.13
  > **[Forward note from P4.17, 2026-08-26]** LibreOffice is the FIRST expected candidate for the §2.12.3 Windows **per-engine tier opt-out** the P4.17 ruling decided up front: a child confined to the intermediate integrity level is refused Medium-labelled DEVICE objects, so it can open no socket and **no named pipe** — and LibreOffice headless's UNO IPC rides a named pipe. Measured at P4.17 on a real confined child (a System32 `ping` fails; `exec`, `>nul` and every write into a granted sink succeed). If the P7 G31/G32 office corpus shows LibreOffice breaking under Leg A, the sanctioned answer is to SKIP Leg A for THIS engine (a per-engine tier decision) — never to widen the grant, and never to label the user's destination directory. The same class covers an engine that DELETEs and RE-CREATEs its output `.part` mid-run (the FFmpeg truncate-vs-recreate shape). Leg B (the Job Object) is unaffected either way.
  ...
  > **[Forward-note from P4.33, 2026-09-15]** the §7.2.1 step-4 executable-permission walk (P4.33) is a named list seam over the P4.33 engine-program table; this box adds the LibreOffice `ResourceBin` row to that table, so the step-4 walk, the P4.42 presence loop and the §7.2.4 "each engine binary" guarantee all cover it.
...
- [ ] **P7.8** [RUST] Wire the LibreOffice invocation SHELL — argv assembly, `--outdir`, headless/nolockcheck/disposable-profile flags · §3.5.2 · G31
  needs: P7.4, P4.73
  > the §3.5.2 argv SHELL (pure Rust dispatch — no filter-name data): `soffice --headless --norestore --nolockcheck --nodefault --nofirststartwizard -env:UserInstallation=file://<per-invocation-profile> --convert-to <ext>:<FilterName>[:<FilterData-JSON>] --outdir <unique-empty-scratch> <input>` — the command-line / arg-vector construction, the `--outdir` slot, the flag set, and the `<FilterName>`/`<FilterData-JSON>` token *positions* (the values come from P7.9/P7.14). P7.10–P7.16 build on this invocation shape. Split from the filter-name table (P7.9): an invocation-shape error fails a different check than a filter-table error, and P7.10–P7.16 need only the invocation shape while P7.14 (FilterData assembly) also needs the filter names.
  > **Plan-time working-dir input (the P4.73 byte-budget ruling (ii)):** the per-job `--outdir` and the per-invocation `-env:UserInstallation` profile this argv embeds live in the item's kind-2 working sub-directory, so this box adds that directory as a plan-time input to `Engine::plan`.
...
- [ ] **P7.10** [RUST] Wire the one-document-per-invocation + per-invocation disposable-profile + §0.9 LibreOffice serialization · §3.5.2 §0.9 §2.6 · G31
  needs: P7.8
  > each LO job gets its **own** disposable `-env:UserInstallation` profile in per-run scratch (§2.14), torn down with the run (§2.6); the §0.9 `serialised_only = true` flag for LibreOffice is read at REGISTRY-BUILD time, where it allocates this engine's dedicated single-permit lane, and the tier-1 conductor acquires that permit BEFORE the global one and holds it across dispatch (parallel instances on one profile lock/corrupt) — one document per invocation, serialized. **[Corrected by P4.22 — the mechanism is BUILT, this box only registers the engine into it]:** the earlier "the pool reads `descriptor()` … and acquires the semaphore before spawn" wording named the wrong layer for both halves (§0.9 `[DECIDED — P4.22]` (a)+(b)); nothing here needs to build a semaphore. **Two P4.22 forward obligations land HERE, because this box registers the FIRST `serialised_only` engine and is therefore the first point at which either is testable:** **(i)** the end-to-end serialisation assertion — two concurrent office jobs must serialise through the real conductor path (today the conductor's guard-holding has NO test, because no registered engine is serialised and `convert_item` reads the `engine_registry()` static, so a `_serialised` → `_` edit would delete serialisation with a green suite); **(ii)** the cancel-under-contention leg for the LANE acquire specifically (see P4.86's obligation — its own drain test cannot reach the office lane before this box exists). The §3.2.2 `EngineDescriptor.serialised_only` data path the pool depends on is populated here for LO.
...
- [ ] **P7.12** [RUST] Wire the LibreOffice exit-0-but-wrote-nothing success verification + the password-protected → §2.8 mapping · §3.5.2 §2.8 · G31
  needs: P7.11
  > the critical correctness rule: LO headless returns **exit 0 even on some failures** and writes nothing → success is verified by the expected output file **existing and being non-empty** in `--outdir`, NOT by exit code. **Zero** new files despite exit 0 → mapped per the stderr rule; encrypted/password files → no output → the §2.8 "password-protected" kind (shared by documents/spreadsheets/presentations); a stale soffice lock avoided by the per-invocation profile + `--nolockcheck`.
...
- [ ] **P7.15** [TEST] Add the per-engine LibreOffice §7.2.3 availability/integrity rows + the in-bundle hash-manifest entries · §7.2.3 · G46 G37
  needs: P7.3, P4.43
  > populate the LibreOffice launcher + program-tree + bundled-font rows in the build-time in-bundle hash manifest and the `EngineHealth` availability table (the per-engine variant of the P4 startup-verifier framework) so a missing/corrupt LibreOffice escalates to a §2.13 app-fault not a crash, and feeds C12 `get_engine_health`.
...
- [ ] **P7.22** [TEST] Add the poppler §7.2.3 availability/integrity row + the in-bundle hash-manifest entry · §7.2.3 · G46 G37
  needs: P7.17, P4.43
  > populate the `pdftotext` row in the in-bundle hash manifest + the `EngineHealth` availability table so a missing/corrupt poppler escalates to a §2.13 app-fault rather than a crash (§3.1 Startup-fault classification, 2026-09-15), and feeds C12 `get_engine_health`.
...
- [ ] **P7.29** [TEST] Add the pandoc §7.2.3 availability/integrity row + the in-bundle hash-manifest entry · §7.2.3 · G46 G37
  needs: P7.23, P4.43
  > populate the `pandoc` row in the in-bundle hash manifest + the `EngineHealth` table so a missing/corrupt pandoc escalates to a §2.13 app-fault rather than a crash (§3.1 Startup-fault classification, 2026-09-15), and feeds C12 `get_engine_health`.
...
- [!extern] **P7.78** [TEST] Run the phase-end Co-Pilot hardening sweep over the whole P7 delivery — adversarial re-test at the hardest technically-possible level · §6.4
  ...
  > **Second leg (§11.4, owner directive 2026-07-22):** the same sweep then pre-fill-audits the P8 plan boxes over every test-strategy §11.4 surface; §11.4 binds, this note only points; resolvable findings land as dual-reviewed plan/spec edits BEFORE the P8 build session starts, genuine forks go to the owner batched at the boundary.
  > **Boundary stop:** P8.1 carries `needs:` on this box — the phase-end sweep box is the one `[!extern]` that blocks its WHOLE successor phase (`_format.md` §2 / §6 step 4, test-strategy §11.3), so the loop hard-stops at the P7→P8 boundary and hands off to the Co-Pilot until the sweep is `[x]`.
