...
- [!extern] **P8.28** [TEST] Run the phase-end Co-Pilot hardening sweep over the whole P8 delivery — adversarial re-test at the hardest technically-possible level · §6.4
  ...
  > **Second leg (§11.4, owner directive 2026-07-22):** the same sweep then pre-fill-audits the P9 plan boxes against the as-built codebase + their cited spec §§ + their dependency edges (incl. embedded-type edges plan-lint cannot see) + information-completeness; resolvable findings land as dual-reviewed plan/spec edits BEFORE the P9 build session starts, genuine forks go to the owner batched at the boundary.
