...
- [x] **P2.80** [RUST] Implement the close-requested decision in Rust — `converter_is_busy` → `api.prevent_close()` + emit `app://close-requested` (`serde_json::Value::Null` payload) · §7.3.2 §7.3.3
  needs: P2.79, P2.39
- [ ] **P2.81** [RUST] Wire the `App::run` `RunEvent::ExitRequested` (last `prevent_exit` chance) + `RunEvent::Exit` (flush logs) handlers — the `.build()?.run(|app, event|)` refactor + the `_ =>` non-exhaustive arm; the best-effort scratch cleanup call is P3.74 · §7.3.2
  needs: P2.78
