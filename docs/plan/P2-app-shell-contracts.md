...
- [x] **P2.71** [RUST] Implement the C2a token-drop-on-EVERY-exit-branch rule (cancelled-dialog → `Empty`, C13-tripped → `Empty`, normal walk-completes) · §1.1 §0.4.4
  needs: P2.70
- [ ] **P2.72** [RUST] Assert the §2.4 freeze idle-vs-in-flight gating contract — the freeze creates a new frozen set (`register` supersedes the prior un-run set, never mutate/merge); busy-refuse stays upstream (the §7.1.1 PRIMARY `forward_launch_intake` funnel + the §5.8 UI defence-in-depth), no core-freeze gate · §1.1 §7.1.1 §2.4
