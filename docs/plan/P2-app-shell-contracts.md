...
- [x] **P2.73** [RUST] Encode the zero-byte/unreadable-at-intake classification — intake-time `Empty`/`Unreadable` = Skipped (pre-flight, never queued); turn-time = Failed (mid-run) · §1.1 §1.2 §0.6
  needs: P2.67, P2.5
- [ ] **P2.74** [RUST] Author the `crate::fs_guard::FileIdentity` resolved-identity type — the §2.3.1 de-dup key (`{ canonical_path, dev_or_volserial, inode_or_fileindex }`, `Eq` + `Hash`); the `resolve_identity` FUNCTION (shell + body) is P3 (P3.1.1 / P3.6) · §2.3.1 §1.1
  needs: P1.11
