...
- [ ] **P2.125** [TEST,RUST] Assert the IPC-responsiveness invariant — no synchronous C-command blocks the WebView past a bound (grouping shell) · §0.4 §1.1 §1.11
  ...
  - [ ] **P2.125.1** [TEST,RUST] Assert the C1/C2a intake commands are async + carry the `on_scan: Channel<ScanProgress>` streaming seam (structural — no sync walk blocks the WebView) · §1.1 §1.11
    needs: P2.38
  ...
  - [ ] **P2.125.2** [TEST,RUST] Assert C3 `get_targets` / C4 `plan_output` / C5 `set_destination` are `pub async fn` (cooperative-yield — no synchronous WebView block) · §0.4 §1.11
    needs: P2.36
