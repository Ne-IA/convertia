...
- [ ] **P4.69** [UI] Build the structural-a11y wiring + the `announcer.ts` body / §5.6.1(2) live-region announce-on-entry on the harness components (ARIA roles + keyboard operability + focus management) · §5.6 §5.6.1 §5.10 · G33a G57
  needs: P4.63, P4.64, P4.65, P4.67, P4.68
  > the §5.6 structural a11y on the P4-built surfaces: ARIA roles + keyboard operability on DropZone/FormatPicker(radiogroup/radio + `aria-checked`)/DestinationBar/OptionsPanel/ProgressList/OpenActions; focus management (focus moved to the new primary element on each state transition — default tile on entering Targets, Convert when DestinationBar appears, Cancel on entering Converting, the §5.6 Summary priority order); roving-tabindex radiogroup; labelled controls; wired via the P1 `a11y/` module (announcer/keymap, §5.6.1/§5.10). **Plus the `announcer.ts` BODY + the §5.6.1(2) announce-on-state-entry wiring** (P1.39 stood up `announcer.ts` interface-only): fill the ARIA-live announcer and fire it per state transition — **ASSERTIVE** on entry for Confirm 3 / RerunPrompt 6 / Summary 8+first-Failed / MixedDropRefusal 9 / UnsupportedNotice 10 / QuitConfirm 11 / AppFault 12; **POLITE** for Collecting progress / lossy-divert notes / throttled Converting milestones (no per-tick flood); `aria-busy` cleared + `aria-valuenow` set to 100/last-known on each item's terminal transition (WCAG 4.1.2) — the distinct-from-focus-management live-region mechanism P9.15 validates. The §6.4.6a jsdom/`vitest-axe` ARIA+focus leg (G33a) binds here.
...
- [ ] **P4.72** [RUST] Build the §1.10 mid-run write-time per-item `Failed(TooBig|OutOfDisk)` enforcement in the §2.1 write sequence · §1.10 §2.1 §2.8 · G31
  > the §1.10 mid-run half (the up-front-vs-mid-run split's per-item leg): at WRITE time, when an item's own size/space breaches the budget (or real disk usage outruns the estimate), the §2.1 write fails, §2.6 restores free space, and the item is reported `Failed(TooBig|OutOfDisk)` (§2.8) **while the batch continues** (§1.9/§1.11 fast-fail surfacing) — there is NO per-item up-front-fail list on `PreflightVerdict`; a per-item doom shows as that item's mid-run terminal row. Plugs the budget check into the P3-built §2.1.1 per-item write sequence (the resolve-late step) + the §2.6 free-space restore. (Build-order: the §2.1 write sequence + §2.6 cleanup are P3's; the fill pass adds the `needs:` on the P3 write-sequence box.)
  needs: P4.71

### P4.16 — Proof-of-life exit gate

- [ ] **P4.73** [TEST] Drive a representative P5 image pair end-to-end through the P4 UX harness (options-panel → progress/cancel → result-actions) · §6.5 §6.4.3 §5.7 · G31 G33a
...
- [ ] **P4.74** [TEST] Verify the P4 proof-of-life exit criterion (imgworker boots + isolated round-trip + populated EngineHealth + first reliability report) · §3.5.5 §2.12 §7.2.3 §6.4.3 · G46 G31
  needs: P4.37, P4.44, P4.59, P4.60, P4.73
  > the consolidated P4 exit gate (README P4 proof-of-life): `convertia-imgworker` boots, a round-trip invocation succeeds through the §2.12 isolation boundary (P4.37), the §7.2.3 startup verifier reports a populated `EngineHealth` (P4.44), the §6.4.3 runner + §6.5.2 pair-status ledger + §6.4.3a bijection guard produce their first report (P4.58–P4.60), AND the UX-harness leg (P4.73) passes — the full P4 "done" predicate.
