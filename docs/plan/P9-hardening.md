...
- [ ] **P9.46** [GATE] Wire the deferred P9→P4 mechanism reconciliation `needs:` edges — §2.12 isolation boundary, §2.12.3 privilege-drop tiers, the egress/fault validations exercise · §2.12 §2.12.3 · G7 G20
  needs: P4.36, P4.15, P4.16, P4.17, P4.18, P4.18.1
  ...
  > **Why P9.46 carries explicit `needs:` while the master P4.76 carries NONE (the two reconciliation-gate models, made explicit):** P4.76 runs a PURELY STRUCTURAL audit (the G20 rule fires on the plan TEXT — it does not build or execute code against the boxes it lists, so it requires none of them `[x]` first). P9.46 is different: the P9 boxes it groups EXECUTE Rust assertions that READ runtime artefacts P4 PRODUCES at build/run time (e.g. the P4.18 `privilege-drop-coverage.toml` the tier-APPLIED regression reads, the P4.36 isolation boundary the fault-injection spawns through), so it takes PHYSICAL `needs:` to guarantee those artefacts exist before the gate runs. Both are valid; the distinction is structural-audit (no needs:) vs runtime-artefact-reading gate (physical needs:) — not an inconsistency.
