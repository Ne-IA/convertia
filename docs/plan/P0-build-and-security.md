...
- [ ] **P0.4.3** [GATE] Lay out the in-core `cargo-fuzz` harness contract (G48 targets) + the per-numeric-IPC-arg overflow leg · §1.2 §6.4.2 · G48 G16
  ...
  > → activated in P1 (the target functions land P1+; harness layout + the date-pinned nightly contract authored now).
...
- [ ] **P0.4.11** [GATE] Define the schema/membership-parity + "every-X-has-a-Y" completeness gate contracts · §0.4 §6.4 · G22 G23 G24
  ...
  > → activated in P1 (the format registry, README support matrix, corpus fixtures, and `convert_*` handlers land in P1; the contracts + self-tests authored now, fail-closing as each target lands).
...
- [ ] **P0.5.8** [TEST] Define the fuzz-crash replay convention — cross-platform stable-toolchain integration test · §6.4.2 · G48 G24
  ...
  > → activated in P1 (the replay harness compiles once the target functions land; convention authored now).
- [ ] **P0.5.9** [TEST] Define the cross-cutting security-test homes — log-redaction / temp-ownership / resource-budget / sentinels / atomicity-under-interruption · §2.1.3 §2.14.1 §6.4.2 §7.5 · G31 G42b
  ...
  > → activated in P3/P4/P9 (each home binds when its mechanism lands — fs_guard P3, isolation P4, the egress window P9; the homes are defined now so no phase re-decides the methodology).
