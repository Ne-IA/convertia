...
- [ ] **P0.4.3** [GATE] Lay out the in-core `cargo-fuzz` harness contract (G48 targets) + the per-numeric-IPC-arg overflow leg · §1.2 §6.4.2 · G48 G16
  ...
  > → activated in P3 (the real fuzz-target function BODIES land in P3 across **P3.6/P3.8** (`crate::fs_guard` `resolve_identity`/`is_safe_output` bodies), **P3.30** (`crate::detect`), **P3.41** (the in-core CSV/TSV engine); the replay harness wiring those bodies into `tests/fuzz_replay.rs` is **P3.67** (which carries `needs: P0.5.8`). P1 ships only interface-only shells, so the harness has nothing instrumentable until P3. The per-`#[tauri::command]` serde-boundary + per-numeric-IPC-arg overflow legs activate in **P2 (P2.126)** as C1–C13 land, the imgworker-FFI leg in **P4 (P4.34.1)** against the real `convertia-imgworker` Rust→FFI surface; harness layout + the date-pinned nightly contract authored now).
...
- [ ] **P0.4.11** [GATE] Define the schema/membership-parity + "every-X-has-a-Y" completeness gate contracts · §0.4 §6.4 · G22 G23 G24
  ...
  > → activated incrementally P4–P7 (the first format-registry entry + `convert_*` handler — CSV→TSV — land in **P3** the walking skeleton, NOT P1, but the G22 bijection-guard script `scripts/check-corpus-coverage.rs` + the README support-matrix row are FIRST stood up in **P4 (P4.59)** — so the CSV/TSV matrix row + G22 first fail-close when **P4.59** lands, NOT in P3; **G23** ("every `convert_*` has a test") fail-closes from **P3** the moment the CSV→TSV `convert_*` handler (the C6 `start_conversion` path, P3.48 + the slice transform P3.41) + its partner test land — the named G23-carrying P3 home is **P3.63** (the per-pair integration runner end-to-end pass, which carries `· G23` and activates this handler→test bijection), a `git ls-files` walk needing no matrix row or bijection script. The README support matrix + corpus fixtures then fill P4–P7 per format; the G22 schema/membership-parity gate fail-closes per format as each matrix row / fixture lands in P4–P7; the contracts + self-tests authored now, fail-open/skip-with-warning while the target is absent.
...
- [ ] **P0.5.9** [TEST] Define the cross-cutting security-test homes — log-redaction / temp-ownership / resource-budget / sentinels / atomicity-under-interruption · §2.1.3 §2.14.1 §6.4.2 §7.5 · G31 G42b
  ...
  > → activated in P2/P3/P4/P9 (each home binds when its mechanism lands — the §7.5 **log-redaction** property gate in **P2 (P2.127)** where the logging infra lands, fs_guard atomicity/temp-ownership in **P3 (P3.19/P3.20/P3.21)**, isolation/privilege-drop (tier-APPLIED-per-spawn regression + §2.12.3 memory-cap kill + process-group/Job-Object reap) in **P4 (P4.18.1)** where the §2.12 wrapper + tiers land, the egress window + sentinels + the macOS-T11/ratchet validation in **P9 (P9.32/P9.37/P9.40/P9.42)**; the **T7/T8 link-safety/self-feeding** home in **P3 (P3.64)**; the **T10 adversarial resource-budget** (1 KB→50 GB intermediate → `Failed(TooBig)`) home in **P9 (P9.41)**; the **Windows AV-retry fault-injection** (§2.1.2 — inject `ERROR_SHARING_VIOLATION` and assert the bounded retry-then-`WriteFailed` path) home in **P3 (the P3.14 retry build + the P3.19 interruption-test family)**; the homes are defined now so no phase re-decides the methodology).
