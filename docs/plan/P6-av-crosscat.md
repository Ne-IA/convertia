...
- [ ] **P6.3** [BUILD] Assert the FFmpeg GPL license posture — `--enable-gpl`, hard-fail `--enable-nonfree`/`libfdk_aac` · §3.6.1 §6.1.3 · G38
  needs: P6.1, P0.7.4
...
- [ ] **P6.4** [BUILD] Build + assert the curated decoder/muxer coverage (generated `ffmpeg-required-decoders.lock`) · §6.1.3 §3.4.3 · G38
  ...
  > **The allowed-decoders golden [Co-Pilot ruling 2026-09-15 — owner may overturn]:** no box generates `ffmpeg-allowed-decoders.lock`, which G38 treats as a generated manifest and P6.43 consumes. This box owns its generator beside `ffmpeg-required-decoders.lock`; the source of the allowed set is decided at the fill against G38 and vuln-response.
...
- [ ] **P6.15** [RUST] Wire the audio-source detection signatures + codec-in-container disambiguation · §1.2 · G15 G31
  needs: P6.10
  > add the §1.2 per-format audio signatures: MP3 frame-sync `FF Fx`/ID3 (layer-disambiguated from MP1/MP2); WAV RIFF/WAVE + `fmt ` chunk; FLAC `fLaC`; raw-ADTS AAC `FF F1`/`FF F9` (ADTS-vs-MP3 by header parse); M4A `ftyp` reading codec from `stsd`/`esds` (AAC ⇒ "M4A", ALAC ⇒ "ALAC"); OGG `OggS`+`\x01vorbis` vs OPUS `OggS`+`OpusHead`; AIFF `FORM`/`AIFF`/`AIFC`; ALAC = M4A `ftyp` + `alac` in `stsd`; WMA ASF GUID + stream-properties codec id. Codec id is authoritative over extension (a `.mp3` that is really FLAC, a `.m4a` that is really ALAC). **KAT-entry deliverable (same commit, G15):** for EACH audio format added here, add a `tests/detect-kat.toml` entry pinning that format's canonical fixture to its exact `FormatId` (+ the genuinely-ambiguous container-disambiguation cases: the **M4A-AAC↔ALAC** shared-`ftyp` `stsd`-codec split, ADTS-AAC-vs-MP3, OGG-Vorbis-vs-OPUS) so a mis-wired container-disambiguation (an ALAC classified as M4A-AAC) is caught at L2 by the G15 KAT test, not only at the per-pair L4 corpus run; the per-format-≥1-KAT-entry completeness is asserted by the P4.60.5 gate.
...
- [ ] **P6.46** [RUST] Wire the video-source detection signatures + brand/DocType disambiguation · §1.2 · G15 G31
  needs: P6.45
  > add the §1.2 video signatures: MP4-family `ftyp` with brand disambiguation (`isom`/`mp4x`/`avc1` = MP4, `qt  ` = MOV, `M4V `/`M4VH` = M4V, `3gp4`/`3g2a` = 3GP — brand not extension); MOV `moov`/`mdat`/`wide` for ftyp-less QuickTime; MKV/WEBM EBML `1A 45 DF A3` disambiguated by DocType (`matroska` vs `webm`); AVI RIFF+`AVI `; WMV ASF GUID + video-stream-present (vs WMA audio-only); FLV `FLV`+version; MPG/MPEG start codes `00 00 01 BA`/`B3` covering the full `.mpg`/`.mpeg`/`.m1v`/`.m2v`/`.vob` (DVD) extension set (video.md:297) + `.ts` sync `0x47` (transport stream, treated here) — the legacy `.vob`/`.m1v`/`.m2v` extensions pinned so none is silently dropped. **KAT-entry deliverable (same commit, G15):** for EACH video format added here, add a `tests/detect-kat.toml` entry pinning that format's canonical fixture to its exact `FormatId` (+ the genuinely-ambiguous cases: the **MP4-family `ftyp` brand split** MP4/MOV/M4V/3GP, the EBML **`matroska`-vs-`webm` DocType** split, the WMV-vs-WMA ASF video-stream-present case) so a mis-wired brand/DocType disambiguation (a MOV classified as MP4) is caught at L2 by the G15 KAT test, not only at the per-pair L4 corpus run; the per-format-≥1-KAT-entry completeness is asserted by the P4.60.5 gate.
...
- [ ] **P6.55** [TEST] Wire the worst-case `willReencode` note + the §1.12 actual-disposition summary · §2.9 §2.9.2 §0.4.2 · G31 G32
  needs: P6.47, P6.95
  > **Forward-ref note (DECISION-C ordering inversion):** `needs: P6.95` points at the `OutcomeMsg::Lossy` producer appended at the end of the phase, whose written note this box's §1.12 summary leg asserts; DECISION C builds it first. Acyclic + valid.
...
- [ ] **P6.81** [TEST] Assert the FFmpeg-family advanced-option completeness (every declared option resolves + every pair has a test) · §1.6 §6.4 · G22 G23
  ...
  > the §6.4 completeness wiring for this engine: G22 every FFmpeg-family format ∈ the §04 category format matrices (the `docs/spec/04-formats/` audio/video/cross-category matrices the bijection guard reads — not a README table) ∧ has a corpus fixture ∧ has a round-trip test; G23's conversion-command→test walk (keyed to the §0.4.1 `start_conversion` — the 2026-07-17 P3.63 ruling) stays green, and every FFmpeg pair has a partner test via the §6.4.3 runner / §6.4.3a `covers` bijection; every registered §1.6 advanced-option declaration resolves to a non-empty handler + a UI control on the P4 panel (no orphan declaration, no declared-but-unwired option).
...
- [ ] **P6.92** [GATE] Wire the deferred P6→P4 harness reconciliation `needs:` edges — isolation boundary, §1.7 lifecycle/probe/progress/kill, per-pair runner + ledger + bijection, TCC staging, options-panel shell · §3.5.1 · G7 G20
  needs: P4.13, P4.14, P4.8, P4.9, P4.10, P4.11, P4.12, P4.25, P4.30, P4.43, P4.49, P4.59, P4.60, P4.61, P4.64, P4.74, P4.92, P4.28.1, P0.7.3, P0.7.4, P4.51, P4.69
  > the P6 instance of the cross-phase reconciliation obligation (the master plan-lint forbidden-string check is P4.77; reciprocal of P3.70/P5.74/P7.77/P9.46): declare the load-bearing P6→P4 + P6→P0 edges the FFmpeg-family boxes consume — the FFmpeg staging executes the **P0.7.3 engine-acquisition + allow-list policy** (P6.1) and the per-engine §6.1.3 assertions execute the **P0.7.4 build-assertion policy** (P6.3–P6.7) and register into the **P4.51 §6.1.3 build-assertion framework** (P6.3–P6.7 + the FFmpeg `paletteuse` exposed-parameter assertion P6.94); the from-source curated FFmpeg compile (P6.1.1) fills the **P4.28.1 from-source compilation harness** configure-flag manifest seam (the network-absent `--disable-network` build the P6.3/P6.6/P6.7 §6.1.3 assertions can only pass against — the prebuilt branch cannot); the FFmpeg beside-the-exe dynamic-library relocation (P6.1.2) drives the **P4.30 generic rpath/install_name rewrite mechanism** so the five component libs resolve inside the bundle (the post-relocation G37b closure asserted in P6.1.2); FFmpeg routes through the **P4.13/P4.14 §2.12 isolation wrapper** (P6.8); probe-then-encode + progress + classify + cancel/kill ride the **P4.8/P4.9/P4.10/P4.11/P4.12/P4.49 §1.7 lifecycle** (P6.10–P6.13; P4.12 bounds BOTH the probe + encode legs under the §1.7 watchdog + the exit≠0 → §3.5 classify routing); macOS TCC staging is **P4.25** (P6.79); the per-engine availability row populates the **P4.43 verifier framework** (P6.14); every per-pair test runs on the **P4.59 §6.4.3 runner** (the audio per-output-format tests P6.32–P6.41, the video per-container tests P6.59–P6.63, the cross-category tests P6.76) and the phase gate drives the **P4.60 bijection guard + P4.61 ledger generator** (P6.80); every advanced-option DECLARATION box (the audio declarations P6.84–P6.91, the video P6.50.1, the extract-audio P6.69.1, the to-GIF P6.71.1/P6.71.2/P6.72.1) renders against the **P4.64 OptionsPanel widget dispatch + the P4.74 AdvancedDrawer**; those declarations, P6.18.1 and the P6.28/P6.54 default-target tables register through the **P4.92 registry offer seam**; the §1.12 lossy producer P6.95 fills the **P4.69** `ItemResult.lossy` field. `needs:` these P4 harness boxes here so the §6 selection builds the P4 mechanism first (P4 is `[x]` before the loop reaches P6 — the edges must RESOLVE, not dangle; the inline engine edges on P6.8–P6.80 carry the per-box dependency, this box is the auditable single owner). No P6 box `>`-note defers a `needs:` with the P4.77-forbidden phrasing.

---

## P4 pre-fill audit additions — the `paletteuse` capability assertion and the per-item lossy producer

> Authored by the **Co-Pilot 2026-09-15** from the test-strategy §11.4 pre-fill audit of the P4 boxes; max+1 convention.

- [ ] **P6.94** [BUILD] Author the FFmpeg `paletteuse` exposed-parameter §6.1.3 capability assertion — the four v1 dither modes (`bayer`, `sierra2_4a`, `floyd_steinberg`, `none`) present in the staged `ffmpeg -h filter=paletteuse` · §6.1.3 · G38
  needs: P6.1, P6.1.1, P4.51, P6.71.2
  > the FFmpeg half of §6.1.3's exposed-parameter bullet (the libvips `effort`/`Q` half is P5.15): a P4.51 capability-assertion slot over the staged curated ffmpeg that FAILS the build if the filter or any of the four decided modes is absent. P6.1.1's `--disable-everything` build is the artifact where a filter trim can drop `paletteuse`. The asserted set is checked equal to the P6.71.2 declaration's mode set, so the asserted and UI-exposed sets cannot drift.
- [ ] **P6.95** [RUST] Write the per-item lossy disclosure — the `OutcomeMsg::Lossy` producer that fills `ItemResult.lossy` from the resolved disposition · §2.9.2 §1.12 §0.6
  needs: P6.47, P4.69
  > the producer the P4.69 reason-slot ruling names [Co-Pilot ruling 2026-09-15 — owner may overturn]: for a succeeded lossy item the core renders the §2.9.1 note through the delivered `crate::outcome::lossy_note` into the additive `ItemResult.lossy` field P4.69 adds, never into `reason`, which keeps the residue precedence. The note reflects the disposition §3.5 actually resolved (P6.47), so the §1.12 summary states what happened (§2.9.2). **Multi-kind slot [Co-Pilot ruling 2026-09-15 — owner may overturn]:** the slot carries ONE `OutcomeMsg::Lossy` whose `text` renders the item's resolved co-applying set (P6.47) with the same de-duplication P4.65 applies at offer time, and whose `kind` is that set's first kind in the §2.9.1 catalog table order, so the P4.69 `Option<OutcomeMsg>` wire shape stays as built. No plan box writes a per-item Lossy note into `ItemResult` before this one, and P6.55 is [TEST]-only, so this box carries the pinning `#[test]`s.
...
- [!extern] **P6.93** [TEST] Run the phase-end Co-Pilot hardening sweep over the whole P6 delivery — adversarial re-test at the hardest technically-possible level · §6.4
  ...
  > **Second leg (§11.4, owner directive 2026-07-22):** the same sweep then pre-fill-audits the P7 plan boxes against the as-built codebase + their cited spec §§ + their dependency edges (incl. embedded-type edges plan-lint cannot see) + information-completeness + their mandate-pair & promise coverage (surface (e), added 2026-08-27); resolvable findings land as dual-reviewed plan/spec edits BEFORE the P7 build session starts, genuine forks go to the owner batched at the boundary.
  > **Boundary stop:** P7.1 carries `needs:` on this box — the phase-end sweep box is the one `[!extern]` that blocks its WHOLE successor phase (`_format.md` §2 / §6 step 4, test-strategy §11.3), so the loop hard-stops at the P6→P7 boundary and hands off to the Co-Pilot until the sweep is `[x]`.
