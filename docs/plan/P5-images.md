...
- **P4 ↔ P5:** P4 built the **generic** harness — the image-worker boots + a
  ...
  PatentDisposition → C3 Target.availability` wiring exists (mirrored in `EngineHealth.unavailable_targets`), and the
...
> aggregation model). So the P5 staging boxes carry the G37b dynamic-closure assertion
...
- [ ] **P5.1** [BUILD] Stage the libvips raster core into the image-worker (no copyleft PDF loader) · §3.1 §3.5.5 §6.1.3 · G37 G37b
  ...
  - [ ] **P5.1.1** [BUILD] Compile libvips from source with the curated no-PDF-loader configure flags via the P4.28.1 harness (fills the libvips configure-flag manifest seam) · §6.1.3 §3.1 §3.6.1 · G37
    needs: P4.28.1
    > the from-source curated libvips build the **P5.2 no-copyleft-PDF-loader assertion can only pass against** — a prebuilt distro libvips frequently enables the poppler-glib PDF loader (libvips#2222), so the LGPL-only property is one only a ConvertIA-configured build has: compile libvips through the **P4.28.1 from-source compilation harness** with the curated configure flags (no `pdfload`/`poppler`/`mupdf`/any GPL/AGPL foreign loader; native `gifsave`/cgif ≥ 8.12), filling the P4.28.1 per-engine `libvips.configure.flags` manifest seam so the configure line is the data P5.2 cross-checks; populate the `<libvips-version-triple>` cache key the P5.1 staging reads. (`needs: P4.28.1` for the from-source harness this fills the seam of; the cross-phase edge carried via the P5.74 reconciliation box.) **Forward-note from the P4.28.1 r7 review — the FIRST box to need it hits it:** the harness refuses an install-path option by SHAPE (a normalised name ending in `dir` or `prefix`), which deliberately over-refuses a `*dir` option naming a path the build merely READS (`-DZLIB_INCLUDE_DIR=`, `--srcdir=`, `--with-libdir=`). The release valve is `_READ_PATH_DIR_OPTIONS` in `scripts/compile-engine-asset` — empty by design. **[Corrected 2026-09-02 after the b9b01df cage ruling: NOT caged.]** The r7 note said the script was *proposed* for the L(-1) cage, so a widening would be an owner act; the owner ruled all three cage questions **not caged**, on the input-vs-enforcement split. So adding a name is Loop work — but it WIDENS the install-redirect guard with no mechanical catcher, so declare it with the box and say which read path needs it, rather than discovering it mid-build. **Scope note (the 2026-07-22 P4.34 build-input ruling):** the curated libvips compile runs FIRST in P4 — pulled forward through the P4.28.1 harness for the P4.34 proof-of-life sidecar, filling the `libvips.configure.flags` seam there; this box RE-VERIFIES the curated properties at its P5 station (the P5.2 assertion's artifact) + owns the seam forward — never a second authoring.
...
- [ ] **P5.5** [BUILD] Stage the REQUIRED ImageMagick delegate (permissive, GPL optional-delegates excluded) for BMP load/save · §3.1 §3.5.5 §6.1.3 · G37 G37b
  ...
  - [ ] **P5.5.1** [BUILD] Compile ImageMagick from source as the trimmed GPL-optional-delegate-excluded build via the P4.28.1 harness (fills the ImageMagick configure-flag manifest seam) · §6.1.3 §3.1 §3.6.1 · G37
    needs: P4.28.1
    > the from-source trimmed ImageMagick build the **P5.6 (option b) coder-excluded path + the P5.7 assertion test against** — a stock ImageMagick ships the dangerous coder/delegate set (URL/HTTPS/MSL/MVG/…) and the GPL optional delegates enabled, so the trimmed permissive-only build is one only a ConvertIA-configured build has: compile ImageMagick through the **P4.28.1 from-source compilation harness** with the curated configure flags (`--without-modules`/coder-excluded per the P5.6 option-(b) path; GPL optional delegates excluded so only the ImageMagick-License permissive core stays, §3.6.1), filling the P4.28.1 per-engine `imagemagick.configure.flags` manifest seam so the configure line is the data P5.7 cross-checks; populate the cache key P5.5 staging reads. (Where P5.6 takes option (a) — bundled hardened `policy.xml` over a stock build — this compile box still produces the GPL-delegate-excluded permissive build for §3.6.1 license-cleanliness; the coder lockdown is then policy.xml, not coder-exclusion.) (`needs: P4.28.1` for the from-source harness; the cross-phase edge carried via the P5.74 reconciliation box.) **Scope note (the 2026-07-22 P4.34 build-input ruling):** the trimmed ImageMagick compile runs FIRST in P4 — pulled forward through the P4.28.1 harness for the P4.34 proof-of-life sidecar, filling the `imagemagick.configure.flags` seam there; this box RE-VERIFIES the trimmed/GPL-delegate-excluded properties at its P5 station (the P5.7 assertion's artifact) + owns the seam forward — never a second authoring.
...
- [ ] **P5.9** [BUILD] Stage the x265 HEVC encoder as a dynamically-loaded libheif encoder plugin (GPL, never static-linked) · §3.1 §3.5.5 §3.6.1 §3.4.3 · G37 G38b
  needs: P5.8
  > x265 (GPL-2.0-or-later — verified vs the pinned source `COPYING`; `-or-later` is compatible with the LGPL-3.0 libheif host) shipped as a **dynamically-loaded libheif encoder plugin** `.so`/`.dll`/`.dylib` under `resources`, **never** statically linked into libvips or the MIT core (§3.6 aggregation); behind the §3.4.4a `available` flag — a scalar bool on each (artifact, target-triple) row, with `codec = "heic_hevc"` on every x265 row (the P4.40 pre-fill ruling) (read in P5.32). G38b: the x265 GPL §3 corresponding-source bundle (P5.69). The from-source x265-as-libheif-plugin COMPILE (the dynamic-plugin build that keeps x265 OUT of the static LGPL worker) is the sub-box P5.9.1 below.
  - [ ] **P5.9.1** [BUILD] Compile x265 from source as the dynamically-loaded libheif encoder plugin via the P4.28.1 harness (fills the x265 configure-flag manifest seam) · §6.1.3 §3.1 §3.6.1 · G37 G38b
    needs: P4.28.1
    > the from-source x265 build that produces the **dynamically-loaded libheif encoder plugin** the §3.6.1 aggregation model requires (x265 GPL must NOT statically link into the LGPL-3.0 libheif/libvips worker — a static link would taint the whole worker GPL; the dynamic-plugin boundary is the load-bearing license control): compile x265 through the **P4.28.1 from-source compilation harness** as a shared `.so`/`.dll`/`.dylib` libheif encoder plugin (GPL-2.0-or-later, verified vs the pinned source `COPYING`), filling the P4.28.1 per-engine `x265.configure.flags` manifest seam (`-DENABLE_SHARED=ON` / plugin-build flags); populate the plugin's cache key P5.9 staging reads + the P5.69 GPL §3 corresponding-source bundle. (`needs: P4.28.1` for the from-source harness; the cross-phase edge carried via the P5.74 reconciliation box.)
...
- [ ] **P5.19** [RUST] Wire the per-target saver dispatch onto the P4.35 `Invocation`-equivalent plan + the P4.36 `VipsStdout` emitter · §3.5.5 §3.2.2 §1.11 · G29 G31
  needs: P5.16
  > the worker's save dispatch: a resolved `TargetFmt` → the per-target saver + its params (the savers P5.20–P5.36), filled into the P4.35 `Invocation`-equivalent plan (the §3.5.5 `Shape:` bullet) and run under the P4.36 `VipsStdout` emitter — both P4 deliverables this box calls and never re-authors [re-scoped 2026-09-15 by the P4 pre-fill audit]; the P4 edges ride P5.74.
...
- [ ] **P5.29** [RUST] Wire SVG output-size resolution (intrinsic / viewBox@96DPI) + bundled-font rendering + pathological-size clamp · §3.5.5 §1.10 · G29 G31
  needs: P5.28, P4.72
  > the SVG render-size + font + clamp concerns, decomposed into the size-resolution core (this box body) + the fontconfig sub-box + the clamp sub-box — three independently-failing deliverables (wrong size vs host-font-fallback tofu vs an unclamped OOM are distinct failure modes; _format.md §3.2, dual review once over the combined diff). **Size resolution (this box):** default render at intrinsic `width`/`height`, else viewBox @ **96 DPI**; target-width-in-px + 2×/3× scale shortcut; transparent background (white for JPG/BMP targets).
  - [ ] **P5.29.1** [RUST] Wire the bundled-font fontconfig setup for SVG text (Liberation/Carlito/Caladea + Noto subset, no host-font access, deterministic substitution) · §3.5.5 §3.9.3 · G29 G31
    > SVG text rendered from the **bundled** font set (Liberation/Carlito/Caladea + the Noto subset, §3.9.3) via the worker's own fontconfig pointed at the bundle font dir — **no host-font access** so substitution is deterministic and reproducible across machines (a missing-font SVG renders the same bundled glyphs everywhere, never host tofu). Fails on a host-font leak / non-deterministic substitution, distinct from the size resolution.
  - [ ] **P5.29.2** [RUST] Wire the pathological tiny-viewBox-huge-render clamp against the P4.72 §1.10 budget engine (fail-clearly, not OOM) · §3.5.5 §1.10 · G29 G31
    needs: P4.72
    > a pathological tiny-viewBox-huge-render clamped against the **P4-built §1.10 budget engine (P4.72)** (fail-clearly, not OOM) — this box FEEDS the §1.10 engine its raster-dims input, never re-implements the ceiling; fails on an unclamped allocation, distinct from size resolution + font setup. (`needs: P4.72` — the §1.10 budget engine this feeds; the cross-phase edge carried via the P5.74 reconciliation box.)
...
- [ ] **P5.47** [RUST] Add the per-format image detection signatures to the §1.2 framework (content-sniff, not extension) · §1.2 · G15 G29
  needs: P5.16
  > the magic/structure signatures for every image source (JPG `FF D8 FF`+APPn+`FF D9`; PNG `89504E47…`+APNG `acTL`; WEBP RIFF/WEBP+`VP8 `/`VP8L`/`VP8X`; GIF `GIF87a`/`GIF89a`; BMP `42 4D`+DIB-header-sanity; TIFF `II*\0`/`MM\0*`+BigTIFF; HEIC ISO-BMFF `ftyp` `heic`/`heix`/`heif`/`mif1`/`heis`/`hevc`; AVIF `ftyp avif`/`avis`; ICO `00 00 01 00` — CUR `00 00 02 00` declined; SVG root `<svg`+`.svgz` gunzip) — added to the P3-bootstrapped §1.2 layered detector; the P0.5.7 detect-KAT convention covers the ambiguous cells. **KAT-entry deliverable (same commit, G15):** for EACH image format added here, add a `tests/detect-kat.toml` entry pinning that format's canonical fixture to its exact `FormatId` (+ a known-ambiguous disambiguation case where one exists — e.g. the HEIC/AVIF shared-`ftyp` brand split, the ICO-vs-CUR leader) so a mis-wired detector (e.g. an AVIF classified as HEIC) is caught at L2 by the G15 KAT test, not only at the per-pair L4 corpus run; the per-format-≥1-KAT-entry completeness is asserted by the P4.60.5 gate (every §04 format has ≥1 KAT entry).
...
- [ ] **P5.67** [BUILD] Populate the P5-staged image-stack §3.7.2 `engines.lock` rows (x265/libaom/dav1d) + the CycloneDX SBOM rows + the G35a derived closure, re-verifying the P4.89-authored static-closure rows (libvips/libheif/libde265/librsvg/cgif/libimagequant/ImageMagick) · §3.7.2 §3.6.2 · G35 G35a G36 G37
  needs: P5.1, P5.5, P5.8, P5.9, P5.11, P5.13, P5.3
  > each image engine/component a §3.7.2 row (mandatory `purl` + SHA-256), the SBOM `purl`-keyed rows + the **DERIVED static-link closure** (G35a) for the statically-linked image-worker stack; license hard-fail (G36) confirms only x265 is GPL (the isolated plugin), the rest LGPL/BSD/permissive; per-engine acquisition anchored per P0.7.3. **Scope note (Co-Pilot 2026-09-09, the P4.89 link-input act):** the rows for the P4.34 static closure (libvips, libheif, libde265, librsvg, cgif, libimagequant, ImageMagick + the build closure the link absorbs) are AUTHORED at P4.89 — owner-acked L(-1) acts ahead of this box, because P4.34 cannot fill its configure seam before them; this box adds the P5-staged rows (the x265 plugin with `codec = "heic_hevc"` on every x265 row per the P4.40 pre-fill ruling, libaom/dav1d) + the SBOM rows + the G35a derived closure and RE-VERIFIES the P4.89 rows at their P5 station — never a second authoring (the P5.1.1/P5.5.1 mirror pattern). No `needs:` to P4.89 is declared: P5.1's `needs: P4.81` already holds every P5 box behind the P4 phase-close, which runs only once every other P4 box — P4.89 and P4.34 included — is `[x]`.
...
- [ ] **P5.69** [BUILD] Author the x265 GPL §3 + image-worker LGPL §6 corresponding-source bundle-present §6.1.3 assertion · §6.1.3 §3.6.2 · G38b
  needs: P5.9, P5.67, P4.76
  > the §6.1.3 carve-out ii/iii bundle-presence assertion for the image stack — ship the static image-worker's complete corresponding source + LGPL object files / relink recipe **and** (because it loads the GPL x265 plugin → GPL combined work) the **x265 GPL §3 complete corresponding source + written offer**; the stage step **fails the build** if either source bundle is missing (the §5 T6 row). **One fact, one home (the SBOM-row pattern, _format.md §8):** P4.76 builds the GENERIC relink-carve-out logic for the image-worker code object; this box (P5.69) is the per-engine BUNDLE-PRESENT assertion as the actual x265/image stack stages; P10.21 is the whole-bundle release assemble+assert — each a different artifact at a different build stage, so the near-identical wording is three layers, not a triple-build (`needs: P4.76`, the generic leg this per-engine assertion populates).
...
### Per-source default-target assertion (the §1.5 one-pre-highlighted-default-per-image-source gate)

> The image-category peer of the audio/video/spreadsheet per-source default-target tables
> (P6.28/P6.54/P7.60), the one ungated zero-click surface noted in skeleton review r10. §1.5
> (SSOT *How It Feels* 4) makes "exactly one pre-highlighted default per source" normative and
> images.md "Per-source default target" is a complete 10-row table (JPG→WEBP, PNG→WEBP,
> WEBP→JPG, GIF→PNG, BMP→PNG, TIFF→PNG, HEIC→JPG, AVIF→JPG, ICO→PNG, SVG→PNG). For images
> these defaults are scattered as inline prose across P5.20–P5.36 with no roll-up assertion —
> and the P4.60.2 G61 guard asserts the OPTION-default property ("every declared option of
> every offered pair carries a default"), NOT the per-source default-TARGET property ("every
> image source has exactly one pre-highlighted default target"): genuinely different
> properties (P4.60.2's note is corrected to say images are covered for OPTION-defaults but the
> per-source default-TARGET is asserted here). End-of-phase placement (gap-free `.77`).
...
- [!extern] **P5.78** [TEST] Run the phase-end Co-Pilot hardening sweep over the whole P5 delivery — adversarial re-test at the hardest technically-possible level · §6.4
  ...
  > **Second leg (§11.4, owner directive 2026-07-22):** the same sweep then pre-fill-audits the P6 plan boxes against the as-built codebase + their cited spec §§ + their dependency edges (incl. embedded-type edges plan-lint cannot see) + information-completeness + their mandate-pair & promise coverage (surface (e), added 2026-08-27); resolvable findings land as dual-reviewed plan/spec edits BEFORE the P6 build session starts, genuine forks go to the owner batched at the boundary.
  > **Boundary stop:** P6.1 carries `needs:` on this box — the phase-end sweep box is the one `[!extern]` that blocks its WHOLE successor phase (`_format.md` §2 / §6 step 4, test-strategy §11.3), so the loop hard-stops at the P5→P6 boundary and hands off to the Co-Pilot until the sweep is `[x]`.
