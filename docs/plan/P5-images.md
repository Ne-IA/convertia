...
- [ ] **P5.8** [BUILD] Stage libheif + libde265 (HEVC decode) for HEIC read · §3.1 §3.5.5 §6.1.3 §3.4.3 · G37 G37b
  needs: P5.1
  > libheif (LGPL-3.0) + libde265 (LGPL-3.0) as the libvips HEIC **load** module (decode-only — §3.4.3 image HEVC-decode row is ship-bundled all platforms); pinned in `engines.lock`, dynamic-closure asserted; SPDX-expression validated (P5.63).
...
- [ ] **P5.13** [BUILD] Stage librsvg (>= 2.56.3 floor) for direct rsvg::Loader SVG load · §3.1 §3.5.5 §6.1.3 · G37 G37b
  needs: P5.1
  > librsvg (LGPL-2.1+) staged inside the image-worker for the **direct `rsvg::Loader`** path (NOT libvips `svgload`); pinned **`>= 2.56.3`** in `engines.lock` (the CVE-2023-38633 belt-and-suspenders floor, not load-bearing for v1 since P5.27 sets no base URL), dynamic-closure asserted.
- [ ] **P5.14** [BUILD] Add the librsvg version-floor + rsvg::Loader API-presence §6.1.3 assertions · §3.5.5 §6.1.3 · G38
  needs: P5.13
  > the stage-step assertions that the staged librsvg is **>= 2.56.3** (fail if older) **and** that the pinned `librsvg` crate/version exposes the relied-upon `rsvg::Loader::read_stream`/`from_data`-without-`base_file` path P5.27 depends on.
- [ ] **P5.15** [BUILD] Author the image-stack exposed-parameter capability §6.1.3 assertions (webpsave/heifsave effort + Q, jpegsave/pngsave/tiffsave/gifsave args) · §3.5.5 §6.1.3 · G38
  ...
  > the image-specific list plugged into the P4 capability-assertion framework — assert the staged libvips actually exposes every per-format knob P5.32–P5.40 declare (`jpegsave` Q/chroma/progressive/optimize_coding, `pngsave` compression/interlace/palette, `webpsave` Q/lossless/effort/alpha_q/near_lossless/smart_subsample, `tiffsave` compression/predictor/tile/pyramid, `gifsave` dither-amount/bitdepth/effort, **`heifsave` `effort`+`Q`** for HEIC+AVIF) — a version bump silently dropping a knob **fails the build**. (The `heifsave effort` arg-presence check is necessary-but-not-sufficient for HEIC exposure — the steer-confirmation corpus spike P5.41 decides HEIC exposure.)
...
- [ ] **P5.16** [RUST] Wire the image-worker load step — by detected type (not extension), inside the §2.12 boundary · §3.5.5 §1.2 §2.12 · G29 G31
  needs: P5.1
  > the worker's load dispatch keyed on the §1.2-detected `FormatId` (built P3, per-format image signatures added P5.42), routing to the right libvips loader / load module / direct rsvg path; runs inside the P4 §2.12 isolation boundary; `VIPS_BLOCK_UNTRUSTED=1` whitelisted in the worker env as defence-in-depth for the non-SVG loaders (NOT load-bearing — the process boundary is, §3.5.5 control 3).
...
- [ ] **P5.26** [BUILD] Run the §6.1.3 ICO multi-size/256px build spike (magicksave write valid `[16,32,48,256]` .ico) + record outcome · §3.5.5 §6.1.3 · G38
  needs: P5.5
  > the `[DEFER: build spike]` resolution — confirm the bundled libvips+ImageMagick `magicksave` can write a valid multi-size `.ico` including a **256px embedded-PNG** entry; **two outcomes:** (a) spike passes → `magicksave` ICO path confirmed and the §6.1.3 assertion fails the build if magicksave ICO save regresses; (b) spike fails → ship the **in-core Rust ICO container assembler** (P5.26.2) and the assertion targets that output, dropping ImageMagick from the ICO path. Record the outcome in this plan's notes.
  - [ ] **P5.26.1** [RUST] Wire the ICO-save path via the spike-confirmed engine (magicksave default; pad-to-square + per-size Lanczos downscale, no-upscale) · §3.5.5 · G29 G31
    needs: P5.26
    > ICO save for `{JPG,PNG,WEBP,GIF,BMP,TIFF,HEIC,AVIF,SVG} → ICO`; default multi-resolution set `[16,32,48,256]`, high-quality Lanczos downscale, **upscale-beyond-source skipped** (note if smaller), non-square **padded to square with transparency** (`[DECIDED]` item 5), 256px stored as embedded PNG; lossy by downscale (`image_downscale`, NOT `image_palette`).
  - [ ] **P5.26.2** [RUST] Build the in-core Rust ICO container assembler fallback (wraps vips per-size PNG/BMP frames) · §3.5.5 · G29 G31
    needs: P5.26
    > the **named fallback if the spike fails** — a safe-Rust ICONDIR + per-entry image-data assembler wrapping vips-produced per-size frames (ICO is a trivial container), removing ImageMagick from the ICO path entirely while keeping vips as the per-frame encoder; the per-frame encode stays one vips process. Built/wired only if P5.26 outcome = (b).

### SVG source path (librsvg, the no-base-URL T9b/SSRF control)

- [ ] **P5.27** [RUST] Wire the SVG load via direct `rsvg::Loader` with NO base_file (the load-bearing T9b/CVE-2023-38633 control) → vips save · §3.5.5 §0.11 §3.3.4 · G29 G31
  needs: P5.16, P5.13
...
- [ ] **P5.30** [RUST] Wire `heifsave compression=hevc` (→ HEIC encode, x265 plugin) — single code path incl. AVIF→HEIC · §3.5.5 §3.4.3 · G29 G31
  needs: P5.19, P5.9, P5.10
  > `heifsave compression=hevc` for `{JPG,PNG,WEBP,GIF,BMP,TIFF,AVIF,ICO,SVG} → HEIC` (incl. the cross-codec `AVIF→HEIC`, one vips process); lossy by default (`image_lossy_codec`), `lossless` toggle; never a default target; gated on the §3.4 HEIC-encode availability cell (P5.31).
- [ ] **P5.31** [RUST] Wire HEIC-encode availability gating — read the §3.4.4a per-platform `available` flag → unavailable-with-reason · §3.4.3 §3.4.4a §2.8 · G29
  needs: P5.30
...
- [ ] **P5.32** [RUST] Wire `heifsave compression=av1` (→ AVIF encode, libaom) — single code path incl. HEIC→AVIF · §3.5.5 §3.4.3 · G29 G31
  needs: P5.19, P5.11
...
- [ ] **P5.33** [RUST] Wire HEIC decode (libheif+libde265 load module) → all raster targets, primary image only · §3.5.5 §3.4.3 · G29 G31
  needs: P5.16, P5.8
...
- [ ] **P5.34** [RUST] Wire AVIF decode (dav1d load module) → all raster targets, primary image only · §3.5.5 §3.4.3 · G29 G31
  needs: P5.16, P5.11
...
- [ ] **P5.35** [RUST] Wire ICO decode (built-in) → all raster targets, largest-frame selection · §3.5.5 · G29 G31
  needs: P5.16
...
> defaults (the `[DEFER: corpus]` ones are confirmed against the corpus, P5.41).

- [ ] **P5.36** [UI] Register the JPG advanced-option declarations (Q=82, chroma auto, progressive on, optimize_coding on, flatten bg) · §1.6 · G47
  needs: P5.20
...
- [ ] **P5.37** [UI] Register the PNG advanced-option declarations (compression=6, interlace off, palette/bitdepth off; palette-only Q100/effort7) · §1.6 · G47
  needs: P5.21
...
- [ ] **P5.38** [UI] Register the WEBP advanced-option declarations (Q=80, lossless off, effort=4, alpha_q=100, near_lossless/smart_subsample/min_size/mixed off) · §1.6 · G47
  needs: P5.22
...
- [ ] **P5.39** [UI] Register the GIF advanced-option declarations (dither = single 0–1 AMOUNT, NOT a mode selector; bitdepth=8, effort=7) · §1.6 · G47
  needs: P5.23
...
- [ ] **P5.40** [UI] Register the BMP + TIFF advanced-option declarations (BMP none; TIFF compression=deflate, Q82-if-jpeg, predictor horizontal, tile/pyramid off) · §1.6 · G47
  needs: P5.24, P5.25
...
- [ ] **P5.41** [UI] Register the HEIC + AVIF advanced-option declarations (HEIC Q=60, lossless off, effort 0–9 corpus-gated; AVIF Q=60, effort=4, lossless off; NO cq-level/preset/speed) · §1.6 · G47
  needs: P5.30, P5.32
  > HEIC basic Q (0–100, default **60**), advanced `lossless` off + integer `effort` 0–9 (default 5) **exposed only if the P5.45 corpus spike confirms `effort` measurably steers the bundled x265/HEVC path — else HIDDEN for HEIC** (no dead control; libheif `speed=9-effort`); AVIF basic Q (0–100, default **60**), advanced `effort` 0–9 (default **4**, libvips-documented as honoured → stays exposed) + `lossless` off; **no `cq-level`/`preset`/`speed` controls** (not heifsave params); 8-bit default, 10/12-bit + chroma 4:2:0 advanced.
- [ ] **P5.42** [UI] Register the SVG advanced-option declarations (width-px / scale 1.0 / explicit WxH / background transparent / dpi 96) · §1.6 · G47
  needs: P5.28
...
- [ ] **P5.43** [UI] Register the category-wide advanced toggles — strip location/metadata (off) + ICC preserve (no sRGB-convert in v1) + flatten background colour · §1.6 · G47
  needs: P5.36
...
- [ ] **P5.44** [RUST] Add the per-format image detection signatures to the §1.2 framework (content-sniff, not extension) · §1.2 · G15 G29
  needs: P5.16
...
- [ ] **P5.45** [TEST] Stage the image corpus into `tests/corpus/images/` + manifest entries with `covers` + SHA-256 manifest · §6.4.5 §6.4.3a · G24a G31
  needs: P5.44
  ...
  - [ ] **P5.45.1** [TEST] Populate every image `covers` 2-tuple so the §6.4.3a bijection guard passes for all 78 image pairs · §6.4.3a §6.4.5 · G31
    needs: P5.45
  ...
  - [ ] **P5.45.2** [TEST] Add the §6.4.5 minimum-content image tags (non-ascii-encoding n/a; orientation/HDR/ICC/animation/alpha exercisers) · §6.4.5 · G31
    needs: P5.45
...
- [ ] **P5.46** [TEST] Per-pair integration tests: → JPG (all sources) — vipsheader decode + dims + orientation-bake + alpha-flatten + lossy-disclosure-iff-flagged · §6.4.3 §6.5 · G31 G32
  needs: P5.20, P5.45.1
...
- [ ] **P5.47** [TEST] Per-pair integration tests: → PNG (all sources) — decode + dims + lossless + APNG-collapse + no-spurious-lossy · §6.4.3 §6.5 · G31 G32
  needs: P5.21, P5.45.1
...
- [ ] **P5.48** [TEST] Per-pair integration tests: → WEBP (all sources) — decode + dims + animation-passthrough + lossy-disclosure · §6.4.3 §6.5 · G31 G32
  needs: P5.22, P5.45.1
...
- [ ] **P5.49** [TEST] Per-pair integration tests: → GIF (all sources) — decode + dims + palette + animation-passthrough + image_palette · §6.4.3 §6.5 · G31 G32
  needs: P5.23, P5.45.1
...
- [ ] **P5.50** [TEST] Per-pair integration tests: → TIFF (all sources) — decode + dims + lossless-default + 16-bit/CMYK fidelity · §6.4.3 §6.5 · G31 G32
  needs: P5.24, P5.45.1
...
- [ ] **P5.51** [TEST] Per-pair integration tests: → BMP (all sources) — decode + dims + 24-bit alpha-flatten (JPG→BMP lossless) · §6.4.3 §6.5 · G31 G32
  needs: P5.25, P5.45.1
...
- [ ] **P5.52** [TEST] Per-pair integration tests: → ICO (all sources) — re-open .ico + assert 16/32/48/256 entries + 256px PNG marker + non-square padding + image_downscale · §6.4.3 §6.1.3 §6.5 · G31 G32
  needs: P5.26.1, P5.45.1
  > each `* → ICO` pair completes, the produced `.ico` is **re-opened and all four [16,32,48,256] entries + the 256px embedded-PNG marker verified** (the runtime proof of whichever ICO path shipped, P5.26), non-square padding to square asserted, `image_downscale` fires; this is the corpus case the §6.1.3 ICO spike (P5.26) points at — **gated on the spike outcome** (magicksave vs the Rust assembler).
- [ ] **P5.53** [TEST] Per-pair integration tests: → HEIC encode (all sources, incl. AVIF→HEIC) — decode + codec + lossy + patent-gap-skip · §6.4.3 §3.4.3 §6.5 · G31 G32
  needs: P5.30, P5.31, P5.45.1
...
- [ ] **P5.54** [TEST] Per-pair integration tests: → AVIF encode (all sources, incl. HEIC→AVIF) — decode + codec + lossy + dav1d-decode-revalidate · §6.4.3 §3.4.3 §6.5 · G31 G32
  needs: P5.32, P5.45.1
...
- [ ] **P5.55** [TEST] Per-pair integration tests: HEIC/AVIF/ICO source → raster targets — primary-image-only + HDR-tonemap + largest-frame · §6.4.3 §6.5 · G31 G32
  needs: P5.33, P5.34, P5.35, P5.45.1
...
- [ ] **P5.56** [TEST] Per-pair integration tests: SVG → {PNG,JPG,WEBP,BMP,TIFF,ICO} — decode + dims + image_svg_raster always + bundled-font render · §6.4.3 §6.5 · G31 G32
  needs: P5.29, P5.45.1
...
- [ ] **P5.57** [TEST] SVG no-base-URL out-of-input / no-egress §6.1.3 corpus case (remote + relative-`../` + absolute `<image href>` not resolved) · §3.5.5 §6.4.2 §0.11 · G31 G32 G42b
  needs: P5.27, P5.45
  > the SVG analogue of the FFmpeg adversarial-egress case — an SVG with an external `<image href>` (relative `../` escape AND absolute AND remote) must **NOT** embed any out-of-input bytes in the output and must trigger **no egress**; with the SVG loaded no-base-URL (P5.27) the reference simply does not resolve. The per-push adversarial-egress pull-forward leg (G42b) is wired in P9; this box stages the SVG sentinel + the out-of-input assertion the corpus run consumes.
- [ ] **P5.58** [TEST] ImageMagick crafted-BMP + SVG-via-MSL/URL-coder sentinel corpus case (no egress, no out-of-input read) · §3.5.5 §6.4.2 §0.11 · G31 G42b
  needs: P5.6, P5.45
...
- [ ] **P5.59** [TEST] Determinism + source-unchanged sub-assertions: >=1 byte-stable pair per saver category + known-non-deterministic AVIF/HEIC manifest exceptions · §2.5 §6.4.3 · G31 G32
  needs: P5.46, P5.47, P5.50
...
- [ ] **P5.60** [TEST] Mark every available image pair `reliable` in the §6.5.2 pair-status ledger on all 3 platforms · §6.5 §6.5.1 §6.5.2 · G31
  needs: P5.46, P5.47, P5.48, P5.49, P5.50, P5.51, P5.52, P5.53, P5.54, P5.55, P5.56
...
- [ ] **P5.61** [TEST] Record the HEIC-encode patent-gap exception (per platform) as a §6.5.3 demoted-pairs / release-note row · §6.5.3 §3.4.3 · G31
  needs: P5.53, P5.60
...
- [ ] **P5.62** [BUILD] Populate the image-stack §3.7.2 `engines.lock` + CycloneDX SBOM rows (libvips/libheif/libde265/x265/libaom/dav1d/librsvg/cgif/libimagequant/ImageMagick) · §3.7.2 §3.6.2 · G35 G35a G36 G37
...
- [ ] **P5.63** [BUILD] Validate the image-stack SPDX expressions + generated-vs-committed NOTICE/THIRD-PARTY-LICENSES parity · §3.7 §6.3.3 · G36 G35
  needs: P5.62
...
- [ ] **P5.64** [BUILD] Author the x265 GPL §3 + image-worker LGPL §6 corresponding-source bundle-present §6.1.3 assertion · §6.1.3 §3.6.2 · G38b
  needs: P5.9, P5.62
  > the §6.1.3 carve-out ii/iii bundle-presence assertion for the image stack — ship the static image-worker's complete corresponding source + LGPL object files / relink recipe **and** (because it loads the GPL x265 plugin → GPL combined work) the **x265 GPL §3 complete corresponding source + written offer**; the stage step **fails the build** if either source bundle is missing (the §5 T6 row).
- [ ] **P5.65** [BUILD] Populate the per-engine §7.2.3 availability rows for the image stack (presence + integrity manifest entries) · §7.2.3 §3.4.4a · G37
  needs: P5.1, P5.62
...
- [ ] **P5.66** [DOC] Record the HEIC `effort`-steers-x265 corpus spike outcome → decide HEIC `effort` exposure (else hidden) · §6.1.3 · G7
  needs: P5.41, P5.45
  > run the `[DEFER: corpus]` spike confirming whether the integer `heifsave effort` measurably steers the bundled x265/HEVC path; **record the outcome** in this plan's notes and flip the P5.41 declaration accordingly — exposed for HEIC iff it steers, **HIDDEN for HEIC** if inert (no dead control; AVIF `effort` stays exposed regardless). The §6.1.3 arg-presence check (P5.15) is necessary but not sufficient — this corpus spike is the exposure decider.
- [ ] **P5.67** [DOC] Confirm the corpus-gated default-Q values (JPG 82 / WEBP 80 / HEIC&AVIF 60) against the real-photo corpus · §6.5 · G7
  needs: P5.45, P5.60
