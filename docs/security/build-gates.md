...
13. **Tauri plugin-surface drift** — the set of `tauri-plugin-*` crates in `Cargo.lock`
    equals an explicit committed allowlist (single-instance, store, log, dialog, opener —
    whatever §0.10 actually grants); any unlisted `tauri-plugin-*` fails (the same
    deny-all-except-allowlist is also wired into `cargo-deny [bans]` at G18 so it fails at
    L2 before a capability JSON exists, closing the `cargo add` → G47 window), and a
    cross-check that the `Cargo.lock` `tauri-plugin-*` set equals the capability-file
    plugin blocks.
...
- **Engine-subprocess coverage-guided fuzz (the highest-risk surface).** Engine-side T1
  ...
  separately-compiled debug build. Mark non-blocking (like G17b) so it can't wedge a
  half-built phase; assign it a `Gnn` from the vacant range when adopted.
...
- **Intermediate T9a structural `std::net` check.** A G29 Semgrep rule failing on
  ...
  post-audit; and a release-tier assertion (alongside G17b) that the RustSec advisory DB
  was refreshed within a bounded window (e.g. 7 days) via `cargo audit` JSON
  `database.last-updated` (a 30-day-stale DB gives false confidence).
