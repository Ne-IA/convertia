...
11. **CI runner-host integrity (the secret never shares a host with untrusted
    ...
    **(b)** the untrusted-corpus/fuzz jobs and the secret-bearing job are
    ...
    Linux-only enforcement) — in **AUDIT** mode on the Lane-B corpus/egress leg (so
    its own egress allowlist + DNS/shim traffic does not pre-empt or distort the
    `connect()` attempts the G42 observe-the-attempt monitor must see), and in
    **BLOCK** mode (strict egress allowlist) only on the secret-bearing signing job,
    which runs no engine and has nothing to observe. This is enforced
    structurally (G56 — a workflow lint flags any secret-using job bound to a
    self-hosted label and asserts the harden-runner mode per job).
...
| CI runner-host integrity (principle 11) | the secret-bearing signing job runs only on an ephemeral GitHub-hosted runner, host-isolated from the untrusted-corpus/fuzz jobs; Linux jobs under `step-security/harden-runner` (**AUDIT** mode on the Lane-B corpus/egress leg so it cannot pre-empt the G42 observe-the-attempt window; **BLOCK** mode reserved for the secret-bearing signing job, which has no engine to observe) | **G56** workflow lint — a secret-using job bound to a self-hosted runner label is a hard fail; the corpus/fuzz job and the signing job assert disjoint runner hosts; harden-runner mode is audit-on-corpus / block-on-signing |
