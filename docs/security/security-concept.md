...
- **§0.11 ↔ §5 parity (closed).** §5 carries one row per §0.11 class. **(FROZEN r1
  snapshot — superseded by the authoritative 19-class set in §5 / §8 / §9 / §16 / plan-lint
  ...
  **T3a** → 15; r3 added **T11** → 16; r15 added **T12** → 17; P2.51 added **T13** → 18; the 2026-07-21 P3.73 P0 ruling added **T7a** → **19**; see §8/§9/§16) — each naming a primary control
...
- **G42b read-half "can never silently no-enforce" scoped to the LINUX leg (honest residual stated).**
  ...
  §2.12.3 tier + the G31 sentinel oracle; both are now **option (b) owner-accepted residual BY DECISION**
  (macOS since the P4.16 cheap-tier ruling, Windows since the P4.17 ruling — the realizable Windows legs confine
  writes/resources only, files carry no `NO_READ_UP`, so no armed planted-positive (a) is reachable) — stated in §5 T9b + G42b.
...
- **Tool corrections (verified against reality).** JS-graph audit `pnpm audit` → **`osv-scanner` over
  `pnpm-lock.yaml`** (`pnpm audit` is online-only — no offline mode; the added "broken on the pinned pnpm
  10.x, HTTP 410" reading was measured false on 10.13.1 on 2026-09-24 and is withdrawn; G17); Windows PE
  mitigation census `checksec`/`dumpbin` → **`winchecksec`**
...
- **Stage 3a (loop-ops / gate / threat cluster).** (a) **Loop liveness + mid-session GitHub-API
