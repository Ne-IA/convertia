...
pub enum InitialDestination {        // C14 get_initial_destination return (§0.4.1 / §5.8) — the returning-user
                                     //   DestinationBar initial state, resolved CORE-side from the persisted
                                     //   §7.4.1 lastDestinationMode; the frontend maps it onto C4's FIRST
                                     //   destination arg (no Last variant, no C4 mirror-back — the P3.80 hand-off)
...
- **`engines`** — the §3.2 `Engine` trait + registry + selection, the §1.7 generic
  invocation lifecycle, §3.5 per-engine arg construction, the §3.3.3
  program-path resolution (`program.rs`, P4.32), and the §3.7.2 `engines.lock`
  schema + validator (`lock.rs`, P4.56.1); every spawn routes
...
│     └─ src/                      # the worker crate source
│
├─ xtask/                          # the `cargo xtask` dev-bin member — §0.4.5 codegen + §6.7.1 coverage bins (G19)
│  └─ src/                         # the xtask bin source
│
├─ fuzz/                           # the `cargo-fuzz` (libFuzzer) crate — the §6.4.2/§2.12.4/G48 in-core untrusted-byte targets (P3.73); a STANDALONE `[workspace]` (its `libfuzzer-sys` dep never enters the MIT-core Cargo.lock/G53 closure/SBOM); path-deps `convertia-core`, drives ONLY `convertia_core::fuzz_api`
│  ├─ fuzz_targets/                # per-target `#![no_main]` harnesses (detect / fs_guard_resolve_identity / fs_guard_is_safe_output / csv_tsv), each importing only `convertia_core::fuzz_api`; zip_slip (P7.50.1) + imgworker_ffi (P4.35.1) join at their boxes (check-fuzz-contract per-target dormancy)
│  └─ corpus/                      # the libFuzzer seed corpus — the committed G16 bound-firing fixtures per target (crashes/ lands minimized crashes as the nightly finds them)
...
│     └─ fs_guard_is_safe_output/    # nul_output_path — the §2.3.3 no-clobber verdict's OWN bound-firing seed (interior-NUL output path → its non-fallback `Err` arm; the fn's fuzz surface is no-panic + never-`Ok(Safe)`-on-unresolvable, never the Windows classes — the P3.73 P0 ruling)
│
...
├─ third-party-licenses/           # the vendored per-component licence texts (one file per component, real copyright lines) the generated THIRD-PARTY-LICENSES.txt reads (§3.7.2 item 2, P4.57)
...
| **csv** | §3.5.6 | the real RFC-4180 reader/writer for the in-core native CSV/TSV transform — parses records at the source delimiter and re-quotes fields containing the target delimiter/quote/newline, preserving field content (incl. leading `= + - @`) literally (the CSV-injection-safe guarantee); pure Rust, no C/C++ decoder (§2.12.4) |
...
| T7a | **Windows reserved-name / trailing-dot-space output aliasing** `[DECIDED — the 2026-07-21 P3.73 P0 ruling]` | A source legally named `CON.csv` (Unix media / network shares) yields the §2.2.1 verbatim-stem candidate `CON.tsv`: a Win32-namespace open of that name aliases the **console/device**, while ConvertIA's own §2.1.2 NT dir-handle-relative publish (no Win32 name parsing) would create a **real** file the user's Win32 apps then cannot open/delete normally; a component ending in a dot/space is silently stripped by the Win32 layer — an alias onto a **different** name. Neither `output_name` (verbatim-stem by §2.2.1 design) nor `is_safe_output` (a pure §2.3.3 no-clobber verdict) owns the class — without a named owner it was an orphan gap | **§2.2.4** Windows-unopenable-name guard: the pre-publish validation seam (beside §2.2.3 `check_path_limit`) rejects — **Windows-only**, on every ConvertIA-**constructed** component (leaf + §2.7.1-recreated subtree dirs, never user-chosen existing ancestors) — a right-trimmed first dot-segment equal to a reserved DOS device name (`CON`/`PRN`/`AUX`/`NUL`/`COM1-9`/`LPT1-9`, case-insensitive) or a trailing dot/space, as a clear per-item §2.8 failure naming the token — never alias/sanitise/truncate; the divert path re-checks identically. Built by **P3.88** | covered (§2.2.4 owns it; the guard landed with **P3.88** — a would-be unopenable output now fails clearly per item, naming the token; before it landed the residual was only an unopenable-but-harmless real file, never a harmed original) |
...
| T11 | **macOS engine-as-first-TCC-accessor (silent-deny)** `[DECIDED — P0 review r3]` | On macOS the source frequently sits in a TCC-protected location (Desktop/Documents/Downloads/removable). If a spawned engine were the FIRST process to touch such a path, a TCC responsible-process chain-break triggers an **invisible denial / wrong-process prompt** that defeats the conversion — and it is **silent on CI** (which runs from `TMPDIR`, where no TCC prompt fires), so a P4/P5 refactor that drops the pre-copy passes CI yet fails for real users on Desktop. **§3.5.0/§7.2.6 call this load-bearing for every macOS engine read.** | **§3.5.0 / §7.2.6 macOS TCC source staging:** the Rust core (which holds the TCC grant, having read the path at §1.1 freeze) copies the source into a **per-job kind-2 scratch path** (§2.14.2) **before** spawning, and hands the sidecar the **scratch path** — so the engine is **never the first process to touch a protected path**. Verifying gate: **G31** macOS sub-test (the Rust core, not the engine PID, is the first accessor; the engine receives a kind-2 scratch path) **and** a **G29** leg with two halves (adjudicated 2026-08-30 — the P4.26 ruling, build-gates G29 (d)): the Semgrep rule (every `Command::new` in the macOS isolation module — the delivered `paths:`-scoped P4.85 form of the `cfg(target_os="macos")` quantifier — is preceded by the stage-for-TCC call), which since the P4.25 §3.2.2 `engine_input` seam has deliberately ZERO in-scope spawn sites and holds vacuously as the armed guard on the module a future mac-conditional spawn is homed into (for the `target_os`/`consts::OS` “macos” spellings the `misplaced_macos_cfg` homing scan matches; a literal-free OS predicate or an audited `nosemgrep` is the G31 leg's to catch); plus the non-vacuous check-sast `t11_seam_pin` (the seam's macOS arm must call the literal standalone `stage_for_tcc` — pinning the LIVE plumbing and the rename class). The behavioural burden sits on the G31 leg. | covered |
