...
| **AppFaultNotice** | post-fault recovery screen | `fault: AppFault`, `onStartOver` | state 12; a chrome heading + **the wire `AppFault.message` rendered VERBATIM** + Start Over → `Idle` (§2.13/§5.8); no stack trace; never fabricates per-item outcomes. **Verbatim-render contract `[DECIDED — re-cut 2026-07-16 by the P3.60 ruling]`:** `message` is the §2.13.3 pre-localised, plain-English, trace-free calm line and **§7.2/§2.13.3 own those words** (§5.7:799 — the UI renders, never paraphrases): `crate::outcome` deliberately refuses §2.8.2 homing for the three app-level kinds ({`EngineMissing`, `WebviewFault`, `BundleDamaged`}) precisely because they "render via the §2.13.3 `app://fault` catalog" — one string, one home — so this component authors **no** body copy and switches on **no** `kind` (a chrome body would leave the §7.2 strings with no renderer anywhere and would tell a damaged-bundle user "the conversion stopped"). Only the heading + the Start-over label are chrome. *(Pre-P3.60 this cell read `onStartOver` alone — a single-class prop list predating the P2.109 `[DECIDED]` block that routes `EngineMissing`/`BundleDamaged` onto this very screen, which would have orphaned `AppFault.message`.)* The **run-path** class (§5.8 Channel-silence / C6-C7 reject) carries **no** DTO — the core is dead and can author nothing — so its chrome line + the machine carrier that admits it are **P4.50**'s, not this component's slice contract |
...
| **App-level fault** (SSOT *Fail clearly*; §2.13.1's third class) `[DECIDED — added 2026-07-16 by the P3.60 ruling]` | `AppFaultNotice` (12): a calm, **trace-free** screen + **Start over** — never a crash dialog, never a fabricated per-item outcome. **Two entry classes, two string owners** (§5.2 row 12): an **`app://fault`** carries the wire `AppFault.message` — rendered **VERBATIM**, since §2.8.2 deliberately does **not** home the three app-level kinds ("render via the §2.13.3 `app://fault` catalog" — one string, one home); the **run-path** fault (Channel silence / C6-C7 reject) carries **no** DTO, so its line is chrome (renderer = P4.50) | **`message` = §7.2/§2.13.3** (verbatim); **the heading, the Start-over label + the run-path line = here (chrome)** |
...
- **WebView/startup faults** (missing engine binary, damaged bundle, WebView fails
  to load) are **startup** concerns whose detection + strings are owned by
  **§7.2/§2.13**. **Their presentation splits by the WebView's own health `[DECIDED —
  precised 2026-07-16 by the P3.60 ruling]`** (§2.13.3's P2.109 block is normative):
  **`EngineMissing`/`BundleDamaged`** leave the WebView healthy, so they are emitted
  over `app://fault` and **DO route into this state machine** — the §5.2 wildcard edge
  → `AppFault` (12), which renders their §7.2-owned `AppFault.message` verbatim;
  only a **`WebviewFault` at startup** (the view itself failed to init) is genuinely
  outside this machine — an `app://fault`→WebView emit is impossible, so it presents
  on a **native** surface and the UI may never mount. *(Pre-P3.60 this bullet put all
  three outside the machine — over-broad against the P2.109 routing, and it would have
  stranded the two healthy-WebView kinds with no renderer.)*
...
- **`app://fault`** → render `AppFaultNotice` (state 12) via the §5.2 wildcard edge,
  rendering the event's **`AppFault.message` VERBATIM** (the §2.13.3/§7.2-owned calm
  line, §5.7) `[DECIDED — precised 2026-07-16 by the P3.60 ruling]`. This class carries
  a DTO, so it does **not** render the disconnect literal above — that literal is the
  **run-path** class's chrome (the bullet above owns that half).
