//! ConvertIA core — the `convertia_core` crate root (the Tauri v2 host, lib + thin bin since P3.87).
//!
//! This file is the CRATE ROOT of the `convertia_core` LIB target (the P3.87 bin+lib split, §0.7 —
//! the P3.73 fuzz lib-target precondition, fork ① of the 2026-07-21 ruling): the §0.7 tier-module
//! declarations, the crate lint policy, the crate-root test suites (declared at the FOOT — the
//! production-prefix slicer's rule; the literal marker is deliberately not written here, see the note
//! beside the module declarations) and the app entry body
//! (`run`, the pre-split `main`) all live here, so `cargo-fuzz` targets can import the crate under test
//! through a path dependency (which resolves the LIB target only). The bin target is the thin
//! `main.rs` shim delegating to `convertia_core::run()` (the standard Tauri-v2 template shape). The
//! public surface is MINIMAL by design: exactly `run` + the dedicated [`fuzz_api`] wrapper module —
//! every §0.7 tier module stays private, so the tier boundaries are not opened as public API.
//!
...
pub mod fuzz_api {
    ...
    /// extension hint (the fuzzer explores content, not names).
    pub fn detect(header: &[u8]) {
        let _ = crate::detection::detect(header);
        if let Some(encoding) = crate::detection::classify_encoding(header) {
            let _ = crate::detection::classify_delimiter(header, encoding, None);
        }
    }
    ...
    /// in-memory sink (`transform_bytes`, the byte-level entry under `csv_tsv_transform`'s file
    /// read), progress/cancel inert. Bounded output relative to input is the transform's own §3.5.6
    /// contract; the wrapper only proves no-panic.
    pub fn csv_tsv_transform(input: &[u8]) {
        for target in [
            crate::engines::CsvTsvTarget::Csv,
            crate::engines::CsvTsvTarget::Tsv,
        ] {
            let mut sink = Vec::new();
            let _ =
                crate::engines::transform_bytes(input, target, &mut sink, &mut |_| {}, &mut || {
                    false
                });
        }
    }
...
/// `Builder::on_window_event(|window, event|)` hook (registered in `run()`) delegates every `WindowEvent`
...
/// signature makes it G28 diff-floor-exempt). `run()`'s closure is a thin delegation line (the established
/// `.setup`/`.plugin` main-body pattern), so the interceptor logic lives in this exempt fn, not in `run()`.
...
/// init, not by this call — so calling this outside `run()` (the test) installs no logger and is
/// side-effect-free.
...
/// This fn returns `()` (internal routing, NOT a short-circuit signal — so `run()`'s setup is unchanged +
...
pub(crate) mod launch_intake {
    ...
    // [Build-Session-Entscheidung: P2.52] The §7.1.1 single-instance callback in `run()` (below) is now the
    ...
    /// `pub(super)` [P2.52]: the funnel's argv door for `run()`'s single-instance callback (the first live caller).
    ...
    /// `init` closure): the boot-stage pattern keeps AppHandle-coupled glue in named fns — `run()` passes it
    ...
    /// boot-glue-exempt). `app.state::<PendingIntake>()` is infallible by construction: `run()` registers the
    ...
    mod tests {
        ...
        // module is LIVE via the single-instance callback in `run()`, so this is now purely a signature pin —
        ...
        // `SecondInstance`; `run()` (covered by `production_main_body`) WIRES it into the single-instance
        ...
        // a second scan pins `run()` REGISTERS the buffer (else the resolve would fail). Needles
        ...
        // queries it; a second scan pins `run()` REGISTERS it. Needles `concat!`-assembled. [Build-Session-Entscheidung: P2.55]
        #[test]
        ...
        // §6.4.1 unit (G15): run() REGISTERS the §0.4.4 RunResultStore (P3.50) so the C8 get_run_summary
        ...
        // reads is_ready; a second scan pins `run()` REGISTERS it (else the resolve would fail). Needles
        ...
        // §6.4.1 unit (G15): run() REGISTERS the §0.4.4 IngestRegistry (P2.70) so the C2a `pick_for_intake`
        // handler's `app.state::<IngestRegistry>()` resolve cannot fail — the store its RAII guard registers
        // the `CollectingId` token in BEFORE opening the native dialog (§1.1). A source-scan on run()'s body
        ...
        // §6.4.1 unit (G15): run() REGISTERS the four §0.4.4/§0.9/§2.5 stores the P3.48 C6 conductor first
        ...
        // `crate::orchestrator::run_conversion` is unit-tested directly), so a source-scan on run()'s body pins
...
mod boot_invariants {
    ...
    /// The production prefix = the crate root `lib.rs` up to the FIRST `cfg(test)` module, so sentinel
    /// needles declared in a test module can never self-match the `include_str!` scan. NOTE: since P2.40
    /// that first `cfg(test)` module is `launch_intake`'s — BEFORE `run()` — so this prefix does NOT reach
    /// `run()`; a scan needing `run()` uses `all_production_source()` (below). `pub(super)`: SHARED with
    /// the §7.3.1 `window_model` scan (P1.16) + `all_production_source()`. Re-pointed `main.rs`→`lib.rs`
    /// by the P3.87 bin+lib split (the app entry body is `run()` in this file; `main.rs` is the thin bin
    /// shim, appended to the corpus by `all_production_source()`).
    ...
    /// `run()`'s body — from the `run()` definition (the app entry body, the pre-P3.87 `main()`) to the
    /// FIRST `cfg(test)` module after it (this one). `production_boot_source()` stops inside
    /// `launch_intake` BEFORE `run()` (P2.40), so it never reaches `run()`'s Builder chain / §7.2.1
    /// startup spine; this slice does. The helper KEEPS its pre-split name (17 consumer sites read
    /// "the app-entry main body"; the slice needle is re-cut to the `run()` definition below).
    /// `pub(super)`: SHARED with `all_production_source()` + the §7.1.1 `single_instance_lock_scope`
    /// registration scan (P2.51). [Build-Session-Entscheidung: P2.54] [Build-Session-Entscheidung: P3.87]
    pub(super) fn production_main_body() -> &'static str {
        let full = include_str!("lib.rs");
        let after_main = full.split_once("fn run()").map_or("", |(_, rest)| rest);
        after_main
            .split_once("#[cfg(test)]")
    ...
    /// sentinel needles never self-match: the pre-test-module prefix PLUS `run()`'s body PLUS the whole
    /// thin `main.rs` bin shim (P3.87 — the shim holds no `cfg(test)` module, so it joins verbatim; a
    /// corpus without it would be blind to logic quietly growing in the bin entry). Needed because
    /// `production_boot_source()` alone stops at `launch_intake`'s test module — BEFORE `run()` — so a
    /// scan over it is blind to `run()`'s Builder chain / startup spine (every item after `run()` is a
    /// `cfg(test)` module, so prefix + entry-body + shim = all production code).
    /// [Build-Session-Entscheidung: P2.54] [Build-Session-Entscheidung: P3.87]
    /// — fixes a pre-existing P2.40 blindness in the `run()`-targeting scans (`boot_path_opens_no_socket`,
    ...
    pub(super) fn all_production_source() -> String {
        format!(
            "{}\n{}\n{}",
            production_boot_source(),
            production_main_body(),
            include_str!("main.rs")
        )
    }
    ...
    // §6.4.1 unit (G15): the P3.87 corpus NON-VACUITY pin — a mis-pointed `include_str!` (a scan left on
    // the thin `main.rs`, or a slice needle missing the moved app entry) would make every source-scan in
    // this file pass VACUOUSLY over an empty/wrong corpus, which is exactly how the bin+lib split could
    // silently disarm the §7.2.2/§7.1/§7.3 invariants. Each slice must still contain a known moved
    // production token, and the appended bin shim must contain its delegation call.
    // [Test-Change: P3.87 — new-test: the split re-pointed the scan corpus lib.rs+main.rs; this pins it
    // non-vacuous, §0.7]
    #[test]
    fn source_scan_corpus_is_not_vacuous() {
        let prefix = production_boot_source();
        assert!(
            prefix.contains("fn register_ipc_identity_types"),
            "P3.87: the lib.rs production prefix lost a known pre-entry token — the boot-source slice is mis-pointed"
        );
        let body = production_main_body();
        assert!(
            body.contains(concat!("tauri_plugin_", "single_instance::init")),
            "P3.87: the app-entry-body slice lost the Builder chain — the entry-body slice is mis-pointed"
        );
        let all = all_production_source();
        // The needle is a main.rs-UNIQUE doc phrase (grep-verified absent from lib.rs), so this leg fails
        // the moment the thin shim leaves the corpus — the delegation call's text alone also appears in
        // this crate doc, which would have satisfied a naive needle from the lib prefix (G1-R1, opus P2).
        assert!(
            all.contains("thin Tauri v2 host-binary entry"),
            "P3.87: the thin main.rs bin shim is missing from the full scan corpus"
        );
    }
    ...
    // `all_production_source()` (prefix + `run()` body) because the §7.2.1 startup spine lives in `run()`,
...
mod instance_identity {
    ...
    //! ESTABLISHMENT in the app entry's `setup` (`run()` since the P3.87 split), which
    //! `production_boot_source()` (truncated before the FIRST `#[cfg(test)]`) does NOT reach — so it scans
    //! the FULL crate root `lib.rs`. [Build-Session-Entscheidung: P2.47]
    ...
    fn instance_id_minted_and_managed_as_a_singleton_in_setup() {
        // Scan the WHOLE crate root (not production_boot_source(), which truncates before the setup
        // closure; lib.rs since the P3.87 split). The needles are concat!-assembled — and the assert
        // MESSAGES deliberately avoid the literal tokens — so the literal never appears in this test file
        // (which include_str! would otherwise self-match, including via a message: a green-but-blind trap).
...
mod window_model {
    ...
    // the pre-test-module prefix + `run()`'s body, every test module excluded so these needles can never
    // self-match) for the Tauri v2 programmatic window-creation constructors. `run()`'s Builder/setup is
...
mod window_lifecycle {
    ...
    // §6.4.1 unit (G15): `run()` WIRES the §7.3.2 `on_window_event` hook and delegates to the named
    // `dispatch_window_event` with the resolved `&AppHandle` (the thin-main-closure pattern). Scans
    // `all_production_source()` (prefix + `run()` body, every `cfg(test)` module excluded so these needles
...
mod run_lifecycle {
    ...
    // §6.4.1 unit (G15): run() registers the run-event handler on the BUILT App (the `.build(ctx)?.run(...)`
...
mod no_updater_posture {
    ...
    // (the shared `boot_invariants` helper: the pre-test-module prefix + `run()`'s body, every test module
    ...
    // The `.plugin(...)` chain lives in `run()`, which `production_boot_source()` alone does not reach (P2.54
...
mod single_instance_lock_scope {
    ...
    // §6.4.1 unit (G15): the production registration site (run()'s Builder chain) DOCUMENTS the per-platform
    // lock scope in-core — the §7.1.1 / T13 note is this box's deliverable, so a source scan pins it cannot
    // silently drop. Scans the shared `crate::boot_invariants::production_main_body()` (run() only, every
    // test module excluded — the `production_main_body` helper was promoted to `boot_invariants` at P2.54 so
    // the `run()`-targeting scans share one source), so these needles — `concat!`-assembled for the same
...
mod log_config {
    ...
    // logger and is side-effect-free — safe to call outside `run()`.
    #[test]
    ...
    // crate a `Debug` `level_for` ceiling (scoped so dependency `debug!` never reaches the file), and run()'s
...
mod startup_spine {
    ...
    // `concat!`-assembled. The `.find` offsets all land in `run()`'s body (the fn DEFS carry `(app:
    ...
    // `.plugin(` registration is the FIRST plugin registration in `run()`'s Builder chain (a registration
...
// `crate::fuzz_replay` outright; when that ruling landed the crate was bin-only, so a cargo `tests/`
// integration target linked no library and could not call the in-core functions it replays (the module's own
// doc records the full reasoning, and the P2.126 IPC-proptest delivery is the precedent). The P3.87 bin+lib
// split has since given the crate a lib target — the crate-root module home simply moved with the crate root
// into this `lib.rs` and remains the delivered shape. It sits under the same at-the-foot rule above.
#[cfg(test)]
mod fuzz_replay;
