...
mod command_surface {
    ...
    //! `start_conversion` (tested in `crate::ipc::conversion`: `c6_contract`), so the 7 remaining C7..C13 shells
    //! are exercised here. [Build-Session-Entscheidung: P2.21]
    ...
    // Listed in §0.4.1 C7..C13 order; C1 `ingest_paths` (P2.22) + C2a `pick_for_intake` (P2.23) are filled and
    ...
    // pre-registry `Err(InternalError)` SHAPE, not its provisional message), so its line moves there and this
    // test now covers the 7 still-bare C7..C13 shells.
    #[test]
