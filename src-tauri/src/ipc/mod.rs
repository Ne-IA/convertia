...
mod command_surface {
    ...
    //! `crate::ipc::planning`: `c2b_contract` / `c3_contract` / `c4_contract` / `c5_contract`), so the 8
    //! remaining C6..C13 shells are exercised here. [Build-Session-Entscheidung: P2.21]
    ...
    // Listed in §0.4.1 C6..C13 order; C1 `ingest_paths` (P2.22) + C2a `pick_for_intake` (P2.23) are filled and
    // tested in `crate::ipc::intake`, and C2b `pick_destination` (P2.24) + C3 `get_targets` (P2.25) + C4
    // `plan_output` (P2.26) + C5 `set_destination` (P2.27) in `crate::ipc::planning`.
    ...
    // (asserting the genuine pre-registry `Err(InternalError)` SHAPE, not its provisional message), so its line
    // moves there and this test now covers the 8 still-bare C6..C13 shells.
    #[test]
