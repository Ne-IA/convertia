...
// nothing constructs one). The §1.1 turn-time `read_failure_to_error_kind` helper is LIVE since P4.25 (the
// conductor's `staging_failure_kind` projects a §3.5.0 staging read failure through it). The rest are dead
// in the PRODUCTION build; the cfg(test) anti-drift +
