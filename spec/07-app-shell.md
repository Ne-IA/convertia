...
> **ImageMagick has no standalone `EngineStatus` `[DECIDED]`.** ImageMagick is a libvips
...
> **inferred from the image-worker's (`EngineId::ImageCore`) health** (if the worker is
> present + runnable, its bundled ImageMagick delegate is too). Its `EngineId::ImageMagick`
